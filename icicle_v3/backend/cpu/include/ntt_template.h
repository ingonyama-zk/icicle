...
namespace ntt_template {
  ...
  public:
    template <typename U>
    friend eIcicleError
    cpu_ntt_init_domain(const Device& device, const U& primitive_root, const NTTInitDomainConfig& config);
    ...
    friend eIcicleError
    cpu_ntt_ref(const Device& device, const E* input, int size, NTTDir dir, NTTConfig<S>& config, E* output);

    template <typename U, typename E>
    friend eIcicleError
    cpu_ntt(const Device& device, const E* input, int size, NTTDir dir, NTTConfig<S>& config, E* output);

    S* get_twiddles() { return twiddles; }
  };
  ...
  eIcicleError cpu_ntt_init_domain(const Device& device, const S& primitive_root, const NTTInitDomainConfig& config)
  {
    ...
    if (s_ntt_domain<S> != nullptr) { return eIcicleError::SUCCESS; }
    // (2) build the domain
        ...
        for (int len = 2; len <= size; len <<= 1) {
          ...
          for (int i = 0; i < size; i += len) {
            for (int j = 0; j < half_len; ++j) {
              ...
              E u = current_temp_elements[i + j];
              E v = current_temp_elements[i + j + half_len] * twiddles[tw_idx];
              // ICICLE_LOG_DEBUG << "tw_idx=" << tw_idx;
              // ICICLE_LOG_DEBUG << "current_temp_elements[" << i + j << "] <-- " << current_temp_elements[i + j] << "
              // + " << current_temp_elements[i + j + half_len] << "*" << twiddles[tw_idx]; ICICLE_LOG_DEBUG <<
              // "current_temp_elements[" << i + j + half_len << "] <-- " << current_temp_elements[i + j] << " - " <<
              // current_temp_elements[i + j + half_len] << "*" << twiddles[tw_idx];
        ...
        for (int len = size; len >= 2; len >>= 1) {
          ...
          for (int i = 0; i < size; i += len) {
            for (int j = 0; j < half_len; ++j) {
              tw_idx = (dir == NTTDir::kForward) ? j * step : size - j * step;
              E u = current_temp_elements[i + j];
              E v = current_temp_elements[i + j + half_len];
              current_temp_elements[i + j] = u + v;
