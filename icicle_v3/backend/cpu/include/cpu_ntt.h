...
namespace ntt_cpu {
  ...
  inline uint64_t idx_in_mem(
    ...
    int block_idx,
    int subntt_idx,
    const std::unique_ptr<std::vector<int>>& layers_sntt_log_size = nullptr,
    int layer = 0)
  {
    ...
    int layer = 0)
  {
    uint64_t subntt_size =
      (layers_sntt_log_size == nullptr) ? 1 << log_original_size : 1 << layers_sntt_log_size->at(layer);
    ...
    icicle::NTTDir dir = icicle::NTTDir::kForward)
  {
    ...
    int i, j;
    int ntt_size = layer == 0
                     ? 1 << (layers_sntt_log_size->at(0) + layers_sntt_log_size->at(1))
                     : 1 << (layers_sntt_log_size->at(0) + layers_sntt_log_size->at(1) + layers_sntt_log_size->at(2));
    int stride = columns_batch ? batch_size : 1;
    for (int batch = 0; batch < batch_size; ++batch) {
  ...
  eIcicleError cpu_ntt(const Device& device, const E* input, uint64_t size, NTTDir dir, NTTConfig<S>& config, E* output)
  {
    ...
    if (logn > 15) {
      ...
      for (int subntt_idx = 0; subntt_idx < (1 << layers_sntt_log_size->at(1)); subntt_idx++) {
        E* current_elements =
          output + stride * (subntt_idx << layers_sntt_log_size->at(0)); // output + subntt_idx * subntt_size
        cpu_ntt_parallel(
          device, (1 << layers_sntt_log_size->at(0)), size, dir, config, current_elements, twiddles, domain_max_size);
      }
      ...
      for (int subntt_idx = 0; subntt_idx < (1 << layers_sntt_log_size->at(0)); subntt_idx++) {
        E* current_elements =
          output + stride * (subntt_idx << layers_sntt_log_size->at(1)); // output + subntt_idx * subntt_size
        cpu_ntt_parallel(
          device, (1 << layers_sntt_log_size->at(1)), size, dir, config, current_elements, twiddles, domain_max_size);
      }
      if (config.columns_batch) {
        reorder_output(output, size, layers_sntt_log_size, config.batch_size, config.columns_batch);
      } else {
