...
namespace ntt_cpu {
  ...
  constexpr uint32_t layers_subntt_log_size[31][3] = {
    ...
    // {1, 0, 0},   {2, 0, 0},   {2, 1, 0},   {2, 2, 0},   {3, 2, 0}, // debug shanie
  ...
  inline uint64_t idx_in_mem(int element, int block_idx, int subntt_idx, const std::unique_ptr<std::vector<int>>& layers_sntt_log_size=nullptr, int layer=0)
  {
    ...
    switch (layer)
    {
      ...
      case 2:
        return ((block_idx << (s1 + s2)) & ((1 << (s0 + s1 + s2)) - 1)) + (((block_idx << (s1 + s2)) >> (s0 + s1 + s2)) << s2) + element;
      default:
  ...
  eIcicleError reorder_by_bit_reverse(int log_original_size, E* elements, int batch_size, bool columns_batch, int block_idx=0, int subntt_idx=0, const std::unique_ptr<std::vector<int>>& layers_sntt_log_size=nullptr, int layer=0)
  {

    uint64_t subntt_size = (layers_sntt_log_size==nullptr)? 1 << log_original_size : 1 << layers_sntt_log_size->at(layer);
    int subntt_log_size = (layers_sntt_log_size==nullptr)? log_original_size : layers_sntt_log_size->at(layer);
  ...
  template <typename S = scalar_t, typename E = scalar_t>
  void dif_ntt(E* elements, uint64_t total_ntt_size, int batch_size, bool columns_batch, const S* twiddles, NTTDir dir, int domain_max_size, int block_idx=0, int subntt_idx=0, const std::unique_ptr<std::vector<int>>& layers_sntt_log_size=nullptr, int layer=0)
  {
    uint64_t subntt_size = 1 << layers_sntt_log_size->at(layer);
    int stride = columns_batch? batch_size : 1;
    for (int batch = 0; batch < batch_size; ++batch) {
    ...
    NTTDir dir = NTTDir::kForward)
  {
    ...
    for (int batch = 0; batch < batch_size; ++batch) {
      ...
      // ICICLE_LOG_DEBUG << "batch=" << batch << ", current_elements=" << current_elements;
      if (arbitrary_coset) {
        ...
        for (int i = 1; i < size; ++i) {
          ...
          // ICICLE_LOG_DEBUG << "current_elements[" << i << "]=" << current_elements[i] << ". twiddles[" << idx << "]=" << twiddles[idx];
  ...
  std::unique_ptr<std::vector<int>> get_layers_subntt_log_size(int log_size)
  {
    ...
    case 7:
      *layers_sntt_log_size = {4, 3, 0};
      break;
    case 6:
      *layers_sntt_log_size = {3, 3, 0}; 
      break;
    // ntt3-5: radix-2
    case 5:
      *layers_sntt_log_size = {5, 0, 0};
      break;
    case 4:
...
*/
  template <typename S = scalar_t, typename E = scalar_t>
  void refactor_and_reorder(E* layer_output, E* next_layer_input, const S* twiddles, int batch_size, bool columns_batch, int domain_max_size, const std::unique_ptr<std::vector<int>>& layers_sntt_log_size=nullptr, int layer=0, icicle::NTTDir dir=icicle::NTTDir::kForward)
  {                 
    ...
    auto temp_elements = std::make_unique<E[]>(ntt_size*batch_size); //TODO shanie - consider using an algorithm for sorting in-place
    int stride = columns_batch? batch_size : 1;
    for (int batch = 0; batch < batch_size; ++batch) {
      ...
      for (int sntt_idx = 0; sntt_idx < nof_sntts; sntt_idx++) {
        for (int elem = 0; elem < sntt_size; elem++) {
          uint64_t tw_idx = (dir == NTTDir::kForward) ? ((domain_max_size / ntt_size) * sntt_idx * elem) : domain_max_size - ((domain_max_size / ntt_size) * sntt_idx * elem);
          cur_temp_elements[stride*(sntt_idx*sntt_size + elem)] = cur_layer_output[stride*(elem * nof_sntts + sntt_idx)] * twiddles[tw_idx];
        }
      }
    }
  ...
  template <typename S = scalar_t, typename E = scalar_t>
  void refactor_output(E* layer_output, E* next_layer_input, uint64_t tot_ntt_size, int batch_size, bool columns_batch, const S* twiddles, int domain_max_size, const std::unique_ptr<std::vector<int>>& layers_sntt_log_size=nullptr, int layer=0, icicle::NTTDir dir=icicle::NTTDir::kForward)
  {
    ...
    int i, j;
    int ntt_size = layer == 0? 1 << (layers_sntt_log_size->at(0) + layers_sntt_log_size->at(1)) : 1 << (layers_sntt_log_size->at(0) + layers_sntt_log_size->at(1) + layers_sntt_log_size->at(2));
    int stride = columns_batch? batch_size : 1;
    for (int batch = 0; batch < batch_size; ++batch) {
  ...
  template <typename S = scalar_t, typename E = scalar_t>
  eIcicleError
  reorder_input(E* input, uint64_t size, int batch_size,bool columns_batch, const std::unique_ptr<std::vector<int>>& layers_sntt_log_size=nullptr){ // TODO shanie future - consider using an algorithm for efficient reordering
  ...
  template <typename S = scalar_t, typename E = scalar_t>
  eIcicleError
  reorder_output(E* output, uint64_t size, const std::unique_ptr<std::vector<int>>& layers_sntt_log_size=nullptr, int batch_size=1, bool columns_batch=0){ // TODO shanie future - consider using an algorithm for efficient reordering
    if (layers_sntt_log_size==nullptr) {
      ICICLE_LOG_ERROR << "layers_sntt_log_size is null";
      return eIcicleError::INVALID_ARGUMENT;
    }
    auto temp_output = std::make_unique<E[]>(size*batch_size);
    uint64_t idx = 0;
    uint64_t mem_idx = 0;
    uint64_t new_idx = 0;
    int subntt_idx;
    ...
    int stride = columns_batch? batch_size : 1;
    for (int batch = 0; batch < batch_size; ++batch) {
      E* current_elements = columns_batch? output + batch : output + batch * size;
      E* current_temp_output = columns_batch? temp_output.get() + batch : temp_output.get() + batch * size;
      for (int i = 0; i < size; i++) {
    ...
    std::copy(temp_output.get(), temp_output.get() + size*batch_size, output);
    return eIcicleError::SUCCESS;
  }
  ...
  cpu_ntt_basic(const icicle::Device& device, E* input, uint64_t original_size, icicle::NTTDir dir, icicle::NTTConfig<S>& config, E* output, int block_idx=0, int subntt_idx=0, const std::unique_ptr<std::vector<int>>& layers_sntt_log_size=nullptr, int layer=0)
  {
    ...
    // NTT/INTT
    if (dit) {
      dit_ntt<S, E>(input, original_size, config.batch_size, config.columns_batch, twiddles, dir, domain_max_size, block_idx, subntt_idx, layers_sntt_log_size, layer); // R --> N
    } else {
      dif_ntt<S, E>(input, original_size, config.batch_size, config.columns_batch, twiddles, dir, domain_max_size, block_idx, subntt_idx, layers_sntt_log_size, layer); // N --> R
    }
  ...
  template <typename S = scalar_t, typename E = scalar_t>
  eIcicleError
  cpu_ntt_parallel(const Device& device, uint64_t size, uint64_t original_size, NTTDir dir, NTTConfig<S>& config, E* output, const S* twiddles, const int domain_max_size = 0)
  {    
    ...
    // ICICLE_LOG_DEBUG << "OUTPUT = " << output;
    for (int layer = 0; layer < layers_sntt_log_size->size(); layer++) {
      ...
      if (layer == 2 && layers_sntt_log_size->at(2)) {
        ...
        for (int block_idx = 0; block_idx < (1<<log_nof_blocks); block_idx++) {
          cpu_ntt_basic(device, output /*input*/, original_size, dir, config, output, block_idx, 0 /*subntt_idx - not used*/, layers_sntt_log_size, layer); //input=output (in-place)
        }
      }
    ...
    if (layers_sntt_log_size->at(1)) { //at least 2 layers
      ...
      //   reorder_output(config.columns_batch? output + b : output + b*original_size, size, layers_sntt_log_size, stride);
      //   ICICLE_LOG_DEBUG << "After reorder_output of batch =" << b;
      //   for (int e=0; e<size*config.batch_size; e++) {
  ...
  eIcicleError cpu_ntt(const Device& device, const E* input, uint64_t size, NTTDir dir, NTTConfig<S>& config, E* output)
  {
    ...
    // Inside each layer each sub-NTT calculation is split to layers as well, and those are calculated in parallel.
