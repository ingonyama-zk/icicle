...
public:
   ...
   */
  ~Msm() { for (std::thread& p3_thread : m_p3_threads) { p3_thread.join(); }}
...
Msm<Point>::Msm(const MSMConfig& config)
    : manager(config.ext->get<int>("n_threads") > 0? config.ext->get<int>("n_threads") :
      ...
      m_buckets(m_num_bms * m_num_bkts), 
      m_bkts_occupancy(m_num_bms * m_num_bkts, false),

      m_log_num_segments(std::max((int)std::floor(
        std::log2((double)(config.ext->get<int>("n_threads") * TASKS_PER_THREAD - 1) / (double)(2 * m_num_bms))), 0)),
      ...
      if (curr_segment.m_nof_received_sums == 2)
      {
        ...
        { 
          ...
          if (m_bkts_occupancy[bkt_idx])
          {
            ...
            // Due to the choice of num segments being less than half of total tasks there ought to be an idle task for the line sum
            task = manager.get_idle_task();
            task->set_phase2_line_addition(curr_segment.line_sum, &m_buckets[bkt_idx], return_idx);
          }
          else { curr_segment.m_nof_received_sums++; } // No need to add a zero - just increase nof_received_sums
        }
      }
...
template <typename Point>
void Msm<Point>::final_accumulator(std::shared_ptr<std::vector<BmSumSegment>>& segments_ptr, int idx_in_batch, Point* result)
{
  // If it isn't the last MSM in the batch - run phase 3 on a separate thread to start utilizing the tasks manager on 
  // the next phase 1.
  if (idx_in_batch == m_batch_size - 1) { phase3_thread(segments_ptr, result); }
  else { m_p3_threads[idx_in_batch] = std::thread(&Msm<Point>::phase3_thread, this, segments_ptr, result); }
}
...
void Msm<Point>::phase3_thread(std::shared_ptr<std::vector<BmSumSegment>> segments_ptr, Point* result)
{
  ...
  for (int i = 0; i < m_num_bms; i++)
  {
    ...
    {
      segments[m_num_bm_segments * i].triangle_sum =  segments[m_num_bm_segments * i].triangle_sum + 
                                                        segments[m_num_bm_segments * i + j].triangle_sum;
    }
    ...
    {
      segments[m_num_bm_segments * i].triangle_sum =  segments[m_num_bm_segments * i].triangle_sum + 
                                                      segments[m_num_bm_segments * i].line_sum;
    }
  }
  ...
  for (int i = m_num_bms - 2; i >= 0; i--)
  {
    ...
    for (int j = 0; j < m_c; j++) { final_sum = Point::dbl(final_sum); }
