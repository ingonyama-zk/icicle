...
int main(int argc, char** argv)
{
  ...
  test_projective::rand_host_many(points, N);
  ...
  config.bases_size = N;
  config.precompute_factor = precomp_factor;
  config.c = user_c;
  ...
  if (precomp_factor > 1)
    msm::cuda_precompute_msm_points<test_affine, test_projective>(points_d, msm_size, config, precomp_points_d);

  // warm up
  msm::msm_cuda<test_scalar, test_affine, test_projective>(
  ...
  // reference
  ConfigExtension ext;
  ext.set(CUDA_MSM_IS_BIG_TRIANGLE, true);
  config.c = 16;
  config.precompute_factor = 1;
  config.batch_size = 1;
  config.bases_size = msm_size;
  config.ext = &ext;
