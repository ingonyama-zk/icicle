...
/**
 * @brief Abstract class representing a hash function. In order to support structures like mmcs, we support side-inputs to the hash. These are inputs that are inserted in an intermediate stage of the calculations.
 */
class Hash {
 public: 
    ...
    const int total_side_input_limbs = 0; //Total number of side input limbs
    ...
    Hash(int total_input_limbs, int total_output_limbs, int total_side_input_limbs = 0)
          ...
          total_output_limbs(total_output_limbs),
     ...
     */
    virtual eIcicleError run_single_hash(const limb_t *input_limbs, limb_t *output_limbs, const HashConfig& config) const = 0;
     ...
     * @return Error code of type eIcicleError.
     */
    virtual eIcicleError run_multiple_hash(const limb_t *input_limbs, limb_t *output_limbs, int nof_hashes, const HashConfig& config, const limb_t *side_input_limbs = nullptr) const = 0;
};