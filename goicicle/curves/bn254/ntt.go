...
func Interpolate(scalars, twiddles, cosetPowers unsafe.Pointer, size int, isCoset bool) {
	...
	var ret C.int
	if isCoset {
		ret = C.interpolate_scalars_on_coset_cuda_bn254(scalarsC, scalarsC, twiddlesC, sizeC, cosetPowersC, 0, 0)
	} else {
		ret = C.interpolate_scalars_cuda_bn254(scalarsC, scalarsC, twiddlesC, sizeC, 0, 0)
	}
	if ret != 0 {
