...
func Ntt(scalars *[]G1ScalarField, isInverse bool, deviceId int) uint64 {
	...
	ret := C.ntt_cuda_bw6761(scalarsC, C.uint32_t(len(*scalars)), C.bool(isInverse), C.size_t(deviceId))
...
func NttBatch(scalars *[]G1ScalarField, isInverse bool, batchSize, deviceId int) uint64 {
	...
	ret := C.ntt_batch_cuda_bw6761(scalarsC, C.uint32_t(len(*scalars)), batchSizeC, isInverseC, deviceIdC)
...
func EcNtt(values *[]G1ProjectivePoint, isInverse bool, deviceId int) uint64 {
	...
	ret := C.ecntt_cuda_bw6761(valuesC, n, isInverseC, deviceIdC)
...
func EcNttBatch(values *[]G1ProjectivePoint, isInverse bool, batchSize, deviceId int) uint64 {
	...
	ret := C.ecntt_batch_cuda_bw6761(valuesC, n, batchSizeC, isInverseC, deviceIdC)
...
func GenerateTwiddles(d_size int, log_d_size int, inverse bool) (up unsafe.Pointer, err error) {
	...
	dp := C.build_domain_cuda_bw6761(domain_size, logn, is_inverse, 0, 0)
...
func ReverseScalars(d_scalars unsafe.Pointer, len int) (int, error) {
	...
	lenC := C.int(len)
	if success := C.reverse_order_scalars_cuda_bw6761(scalarsC, lenC, 0, 0); success != 0 {
...
func Interpolate(scalars, twiddles, cosetPowers unsafe.Pointer, size int, isCoset bool) unsafe.Pointer {
	...
	var ret C.int
	if isCoset {
		ret = C.interpolate_scalars_on_coset_cuda_bw6761(d_out, scalarsC, twiddlesC, sizeC, cosetPowersC, 0, 0)
	} else {
		ret = C.interpolate_scalars_cuda_bw6761(d_out, scalarsC, twiddlesC, sizeC, 0, 0)
	}
	if ret != 0 {
...
func Evaluate(scalars_out, scalars, twiddles, coset_powers unsafe.Pointer, scalars_size, twiddles_size int, isCoset bool) int {
	...
	var ret C.int
	if isCoset {
		ret = C.evaluate_scalars_on_coset_cuda_bw6761(scalars_outC, scalarsC, twiddlesC, twiddlesC_size, sizeC, coset_powersC, 0, 0)
	} else {
		ret = C.evaluate_scalars_cuda_bw6761(scalars_outC, scalarsC, twiddlesC, twiddlesC_size, sizeC, 0, 0)
	}
...
func VecScalarAdd(in1_d, in2_d unsafe.Pointer, size int) int {
	...
	ret := C.add_scalars_cuda_bw6761(in1_dC, in1_dC, in2_dC, sizeC, 0)
...
func VecScalarSub(in1_d, in2_d unsafe.Pointer, size int) int {
	...
	ret := C.sub_scalars_cuda_bw6761(in1_dC, in1_dC, in2_dC, sizeC, 0)
...
func ToMontgomery(d_scalars unsafe.Pointer, len int) (int, error) {
	scalarsC := (*C.BW6761_scalar_t)(d_scalars)
	lenC := C.uint(len)
	if success := C.to_montgomery_scalars_cuda_bw6761(scalarsC, lenC, 0); success != 0 {
		return -1, errors.New("reversing failed")
	}
	return 0, nil
}
...
func FromMontgomery(d_scalars unsafe.Pointer, len int) (int, error) {
	scalarsC := (*C.BW6761_scalar_t)(d_scalars)
	lenC := C.uint(len)
	if success := C.from_montgomery_scalars_cuda_bw6761(scalarsC, lenC, 0); success != 0 {
		return -1, errors.New("reversing failed")
	}
	return 0, nil
}
...
func AffinePointFromMontgomery(d_points unsafe.Pointer, len int) (int, error) {
	...
	if success := C.from_montgomery_aff_points_cuda_bw6761(pointsC, lenC, 0); success != 0 {
		return -1, errors.New("reversing failed")
	}
	return 0, nil
}
...
func G2AffinePointFromMontgomery(d_points unsafe.Pointer, len int) (int, error) {
	...
	if success := C.from_montgomery_aff_points_g2_cuda_bw6761(pointsC, lenC, 0); success != 0 {
		return -1, errors.New("reversing failed")
	}
	return 0, nil
}
