...
func (p *G2PointAffine) FromProjective(projective *G2Point) *G2PointAffine {
	in := (*C.BLS12381_g2_projective_t)(unsafe.Pointer(projective))

	out := C.g2_projective_to_affine_bls12381(in)
...
func (p *G2Point) IsOnCurve() bool {
	// Directly copy memory from the C struct to the Go struct
	point := (*C.BLS12381_g2_projective_t)(unsafe.Pointer(p))
	res := C.g2_projective_is_on_curve_bls12381(point)
