...
func (p *G1ProjectivePoint) IsOnCurve() bool {
	point := (*C.BLS12381_projective_t)(unsafe.Pointer(p))
	res := C.projective_is_on_curve_bls12381(point)

	return bool(res)
}

func (p *G1ProjectivePoint) Random() *G1ProjectivePoint {
	rand := C.random_projective_bls12381()
...
func (p *G1PointAffine) FromProjective(projective *G1ProjectivePoint) *G1PointAffine {
	in := (*C.BLS12381_projective_t)(unsafe.Pointer(projective))

	out := C.projective_to_affine_bls12381(in)
