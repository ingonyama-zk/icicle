...
func NttBatchBLS12381(scalars *[]G1ScalarField, isInverse bool, batchSize, deviceId int) uint64 {
	...
	ret := C.ntt_batch_cuda_bls12381(scalarsC, C.uint32_t(len(*scalars)), batchSizeC, isInverseC, deviceIdC)
...
func EcNttBLS12381(values *[]G1ProjectivePoint, isInverse bool, deviceId int) uint64 {
	...
	ret := C.ecntt_cuda_bls12381(valuesC, n, isInverseC, deviceIdC)
...
func EcNttBatchBLS12381(values *[]G1ProjectivePoint, isInverse bool, batchSize, deviceId int) uint64 {
	...
	ret := C.ecntt_batch_cuda_bls12381(valuesC, n, batchSizeC, isInverseC, deviceIdC)
...
func GenerateTwiddles(d_size int, log_d_size int, inverse bool) (up unsafe.Pointer, err error) {
	...
	dp := C.build_domain_cuda_bls12381(domain_size, logn, is_inverse, 0, 0)
...
func ReverseScalars(d_scalars unsafe.Pointer, len int) (int, error) {
	scalarsC := (*C.BLS12381_scalar_t)(d_scalars)
	lenC := C.int(len)
	if success := C.reverse_order_scalars_cuda_bls12381(scalarsC, lenC, 0, 0); success != 0 {
...
func Interpolate(scalars, twiddles, cosetPowers unsafe.Pointer, size int, isCoset bool) unsafe.Pointer {
	...
	d_out := (*C.BLS12381_scalar_t)(dp)
	scalarsC := (*C.BLS12381_scalar_t)(scalars)
	twiddlesC := (*C.BLS12381_scalar_t)(twiddles)
	cosetPowersC := (*C.BLS12381_scalar_t)(cosetPowers)
	sizeC := C.uint(size)

	var ret C.int
	if isCoset {
		ret = C.interpolate_scalars_on_coset_cuda_bls12381(d_out, scalarsC, twiddlesC, sizeC, cosetPowersC, 0, 0)
	} else {
		ret = C.interpolate_scalars_cuda_bls12381(d_out, scalarsC, twiddlesC, sizeC, 0, 0)
	}
	if ret != 0 {
...
func Evaluate(scalars_out, scalars, twiddles, coset_powers unsafe.Pointer, scalars_size, twiddles_size int, isCoset bool) int {
	scalars_outC := (*C.BLS12381_scalar_t)(scalars_out)
	scalarsC := (*C.BLS12381_scalar_t)(scalars)
	twiddlesC := (*C.BLS12381_scalar_t)(twiddles)
	coset_powersC := (*C.BLS12381_scalar_t)(coset_powers)
	sizeC := C.uint(scalars_size)
	twiddlesC_size := C.uint(twiddles_size)

	var ret C.int
	if isCoset {
		ret = C.evaluate_scalars_on_coset_cuda_bls12381(scalars_outC, scalarsC, twiddlesC, twiddlesC_size, sizeC, coset_powersC, 0, 0)
	} else {
		ret = C.evaluate_scalars_cuda_bls12381(scalars_outC, scalarsC, twiddlesC, twiddlesC_size, sizeC, 0, 0)
	}
...
func VecScalarAdd(in1_d, in2_d unsafe.Pointer, size int) int {
	in1_dC := (*C.BLS12381_scalar_t)(in1_d)
	in2_dC := (*C.BLS12381_scalar_t)(in2_d)
	sizeC := C.uint(size)

	ret := C.add_scalars_cuda_bls12381(in1_dC, in1_dC, in2_dC, sizeC, 0)
...
func VecScalarSub(in1_d, in2_d unsafe.Pointer, size int) int {
	...
	ret := C.sub_scalars_cuda_bls12381(in1_dC, in1_dC, in2_dC, sizeC, 0)
...
func ToMontgomery(d_scalars unsafe.Pointer, len int) (int, error) {
	scalarsC := (*C.BLS12381_scalar_t)(d_scalars)
	lenC := C.uint(len)
	if success := C.to_montgomery_scalars_cuda_bls12381(scalarsC, lenC, 0); success != 0 {
		return -1, errors.New("reversing failed")
	}
	return 0, nil
}
...
func FromMontgomery(d_scalars unsafe.Pointer, len int) (int, error) {
	scalarsC := (*C.BLS12381_scalar_t)(d_scalars)
	lenC := C.uint(len)
	if success := C.from_montgomery_scalars_cuda_bls12381(scalarsC, lenC, 0); success != 0 {
		return -1, errors.New("reversing failed")
	}
	return 0, nil
}
...
func AffinePointFromMontgomery(d_points unsafe.Pointer, len int) (int, error) {
	pointsC := (*C.BLS12381_affine_t)(d_points)
	lenC := C.uint(len)

	if success := C.from_montgomery_aff_points_cuda_bls12381(pointsC, lenC, 0); success != 0 {
		return -1, errors.New("reversing failed")
	}
	return 0, nil
}
...
func G2AffinePointFromMontgomery(d_points unsafe.Pointer, len int) (int, error) {
	pointsC := (*C.BLS12381_g2_affine_t)(d_points)
	lenC := C.uint(len)

	if success := C.from_montgomery_aff_points_g2_cuda_bls12381(pointsC, lenC, 0); success != 0 {
		return -1, errors.New("reversing failed")
	}
	return 0, nil
}
