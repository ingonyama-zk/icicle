...
func (p *G1ProjectivePoint) IsOnCurve() bool {
	point := (*C.BLS12377_projective_t)(unsafe.Pointer(p))
	res := C.projective_is_on_curve_bls12377(point)
...
func (p *G1PointAffine) FromProjective(projective *G1ProjectivePoint) *G1PointAffine {
	in := (*C.BLS12377_projective_t)(unsafe.Pointer(projective))

	out := C.projective_to_affine_bls12377(in)
