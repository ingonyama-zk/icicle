...
func testAgainstGnarkCryptoMsm(scalars core.HostSlice[bn254Fields.ScalarField], points core.HostSlice[Affine], out Projective) bool {
...
func testAgainstGnarkCryptoMsmGnarkCryptoTypes(scalarsFr core.HostSlice[fr.Element], pointsFp core.HostSlice[bn254.G1Affine], out Projective) bool {
...
func convertIcicleAffineToG1Affine(iciclePoints []Affine) []bn254.G1Affine {
...
func TestMSM(t *testing.T) {
	...
	for _, power := range []int{2, 3, 4, 5, 6, 7, 8, 10, 18} {
		...
		e = Msm[bn254Fields.ScalarField, Affine](scalars, points, &cfg, out)
...
func TestPrecomputeBase(t *testing.T) {
	...
	for _, power := range []int{10, 16} {
		for _, batchSize := range []int{1, 3, 16} {
			...
			e = PrecomputeBases[Affine](points, precomputeFactor, 0, &cfg.Ctx, precomputeOut)
			...
			e = Msm[bn254Fields.ScalarField, Affine](scalars, precomputeOut, &cfg, out)
			assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
			outHost := make(core.HostSlice[Projective], batchSize)
			outHost.CopyFromDevice(&out)
			out.Free()
