...
func projectiveToGnarkAffineG2(p G2Projective) gnarkBn254.G2Affine {
	...
	g2Jac := gnarkBn254.G2Jac{
		X: X,
		Y: Y,
		Z: z,
	}

	var g2Affine gnarkBn254.G2Affine
	return *g2Affine.FromJacobian(&g2Jac)
}

func testAgainstGnarkCryptoMsmG2(scalars core.HostSlice[bn254.ScalarField], points core.HostSlice[G2Affine], out G2Projective) bool {
	...
	pointsFp := make([]gnarkBn254.G2Affine, len(points))
...
func testAgainstGnarkCryptoMsmG2GnarkCryptoTypes(scalarsFr core.HostSlice[fr.Element], pointsFp core.HostSlice[gnarkBn254.G2Affine], out G2Projective) bool {
	var msmRes gnarkBn254.G2Jac
	msmRes.MultiExp(pointsFp, scalarsFr, ecc.MultiExpConfig{})

	var icicleResAsJac gnarkBn254.G2Jac
...
func convertIcicleG2AffineToG2Affine(iciclePoints []G2Affine) []gnarkBn254.G2Affine {
	points := make([]gnarkBn254.G2Affine, len(iciclePoints))
	for index, iciclePoint := range iciclePoints {
		...
		points[index] = gnarkBn254.G2Affine{
			X: gnarkBn254.E2{
				A0: xA0Elem,
				A1: xA1Elem,
			},
...
func TestMSMG2(t *testing.T) {
	...
	for _, power := range []int{2, 3, 4, 5, 6, 7, 8, 10, 18} {
		...
		scalars := bn254Fields.GenerateScalars(size)
		...
		e = G2Msm[bn254.ScalarField, G2Affine](scalars, points, &cfg, out)
...
func TestMSMG2GnarkCryptoTypes(t *testing.T) {
	...
	for _, power := range []int{3} {
		...
		pointsHost := (core.HostSlice[gnarkBn254.G2Affine])(pointsGnark)
		...
		e = G2Msm[fr.Element, gnarkBn254.G2Affine](scalarsHost, pointsHost, &cfg, out)
		assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
		outHost := make(core.HostSlice[G2Projective], 1)
...
func TestMSMG2Batch(t *testing.T) {
	...
	for _, power := range []int{10, 16} {
		for _, batchSize := range []int{1, 3, 16} {
			...
			scalars := bn254Fields.GenerateScalars(totalSize)
...
func TestPrecomputeBaseG2(t *testing.T) {
	...
	for _, power := range []int{10, 16} {
		for _, batchSize := range []int{1, 3, 16} {
			...
			e = G2Msm[bn254.ScalarField, G2Affine](scalars, precomputeOut, &cfg, out)
			assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
			outHost := make(core.HostSlice[G2Projective], batchSize)
			outHost.CopyFromDevice(&out)
			out.Free()
