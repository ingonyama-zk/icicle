...
if [[ $1 == "-help" ]]; then
  ...
  echo ""
  echo "  -skip_poseidon2           Builds the curve or field library with poseidon2 hashing disabled."
  echo ""
...
do
  ...
  cmake -DCMAKE_CUDA_COMPILER=$CUDA_COMPILER_PATH \
        -DCUDA_BACKEND=$CUDA_BACKEND \
        -DCURVE=$CURVE \
        -DMSM=$MSM_DEFINED \
        -DNTT=$NTT_DEFINED \
        -DG2=$G2_DEFINED \
        -DECNTT=$ECNTT_DEFINED \
        -DPOSEIDON=$POSEIDON \
        -DPOSEIDON2=$POSEIDON2 \
        -DHASH=$HASH \
