...
import (
	...
	"github.com/ingonyama-zk/icicle/v3/wrappers/golang/fields/stark252/polynomial"
	"github.com/ingonyama-zk/icicle/v3/wrappers/golang/fields/stark252/vecOps"
	"github.com/ingonyama-zk/icicle/v3/wrappers/golang/internal/test_helpers"
	"github.com/stretchr/testify/suite"
)

var one, two, three, four, five stark252.ScalarField
...
func rand() stark252.ScalarField {
	return stark252.GenerateScalars(1)[0]
}

func randomPoly(size int) (f polynomial.DensePolynomial) {
	f.CreateFromCoeffecitients(core.HostSliceFromElements(stark252.GenerateScalars(size)))
	return f
}

func vecOp(a, b stark252.ScalarField, op core.VecOps) stark252.ScalarField {
...
func testPolyCreateFromCoefficients(suite *suite.Suite) {
	scalars := stark252.GenerateScalars(33)
...
func testPolyEval(suite *suite.Suite) {
	...
	coeffs := core.HostSliceFromElements([]stark252.ScalarField{five, two, four})
	var f polynomial.DensePolynomial
	f.CreateFromCoeffecitients(coeffs)

	var x stark252.ScalarField
	x.FromUint32(8)
	...
	var expected stark252.ScalarField
	suite.Equal(expected.FromUint32(277), fEvaled.(core.HostSlice[stark252.ScalarField])[0])
}
...
func testPolyMonomials(suite *suite.Suite) {
	var zero stark252.ScalarField
	var f polynomial.DensePolynomial
	f.CreateFromCoeffecitients(core.HostSliceFromElements([]stark252.ScalarField{one, zero, two}))
	x := rand()
...
func testPolyReadCoeffs(suite *suite.Suite) {
	var f polynomial.DensePolynomial
	coeffs := core.HostSliceFromElements([]stark252.ScalarField{one, two, three, four})
	f.CreateFromCoeffecitients(coeffs)
	coeffsCopied := make(core.HostSlice[stark252.ScalarField], coeffs.Len())
	...
	coeffsHost := make(core.HostSlice[stark252.ScalarField], coeffs.Len())
...
func testPolyOddEvenSlicing(suite *suite.Suite) {
	...
	x := rand()
	var evenExpected, oddExpected stark252.ScalarField
	for i := size; i >= 0; i-- {
		if i%2 == 0 {
...
func testPolynomialDivision(suite *suite.Suite) {
	...
	f.CreateFromCoeffecitients(core.HostSliceFromElements(stark252.GenerateScalars(1 << 4)))
	g.CreateFromCoeffecitients(core.HostSliceFromElements(stark252.GenerateScalars(1 << 2)))
	...
	x := stark252.GenerateScalars(1)[0]
	fEval := f.Eval(x)
...
func testDivideByVanishing(suite *suite.Suite) {
	...
	coeffs := core.HostSliceFromElements([]stark252.ScalarField{minus_one, zero, zero, zero, one}) // x^4-1
...
// 	size := 4
// 	coeffs := stark252.GenerateScalars(size)
...
// 	hostSlice := make(core.HostSlice[stark252.ScalarField], size)
// 	hostSlice.CopyFromDevice(fSlice)
// 	suite.Equal(coeffs, hostSlice)

// 	cfg := ntt.GetDefaultNttConfig()
// 	res := make(core.HostSlice[stark252.ScalarField], size)
