...
func testNTTGetDefaultConfig(suite *suite.Suite) {
	...
	expected := test_helpers.GenerateLimbOne(int(stark252.SCALAR_LIMBS))
...
func testNtt(suite *suite.Suite) {
	...
	scalars := stark252.GenerateScalars(1 << largestTestSize)
	...
	for _, size := range []int{4, largestTestSize} {
		for _, direction := range []core.NTTDir{core.KForward, core.KInverse} {
			for _, v := range [4]core.Ordering{core.KNN, core.KNR, core.KRN, core.KRR} {
				...
				scalarsCopy := core.HostSliceFromElements[stark252.ScalarField](scalars[:testSize])
				cfg.Ordering = v

				// run ntt
				test_helpers.ActivateReferenceDevice()
				output := make(core.HostSlice[stark252.ScalarField], testSize)
				ntt.Ntt(scalarsCopy, direction, &cfg, output)

				test_helpers.ActivateMainDevice()
				outputMain := make(core.HostSlice[stark252.ScalarField], testSize)
...
func testNttBatch(suite *suite.Suite) {
	...
	largestTestSize := 10
	largestBatchSize := 20
	scalars := stark252.GenerateScalars(1 << largestTestSize * largestBatchSize)
	...
	for _, size := range []int{4, largestTestSize} {
		for _, batchSize := range []int{2, 16, largestBatchSize} {
			...
			scalarsCopy := core.HostSliceFromElements[stark252.ScalarField](scalars[:totalSize])
			...
			// Ref device
			test_helpers.ActivateReferenceDevice()
			output := make(core.HostSlice[stark252.ScalarField], totalSize)
			ntt.Ntt(scalarsCopy, core.KForward, &cfg, output)

			// Main device
			test_helpers.ActivateMainDevice()
			outputMain := make(core.HostSlice[stark252.ScalarField], totalSize)
