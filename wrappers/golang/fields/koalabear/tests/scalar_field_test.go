...
const (
	SCALAR_LIMBS = koalabear.SCALAR_LIMBS
)

func testScalarFieldFromLimbs(suite *suite.Suite) {
	emptyField := koalabear.ScalarField{}
...
func testScalarFieldOne(suite *suite.Suite) {
	var emptyField koalabear.ScalarField
	emptyField.One()
...
func testKoalabearGenerateScalars(suite *suite.Suite) {
	const numScalars = 8
	scalars := koalabear.GenerateScalars(numScalars)

	suite.Implements((*core.HostOrDeviceSlice)(nil), &scalars)

	suite.Equal(numScalars, scalars.Len())
	zeroScalar := koalabear.ScalarField{}
...
func testScalarFieldArithmetic(suite *suite.Suite) {
	...
	scalarsA := koalabear.GenerateScalars(size)
	scalarsB := koalabear.GenerateScalars(size)
	...
	one := scalarA.Mul(&inv)
	expectedOne := koalabear.GenerateScalars(1)[0]
	expectedOne.One()

	suite.Equal(expectedOne, one)
}

func testKoalabearMongtomeryConversion(suite *suite.Suite) {
	size := 1 << 20
	scalars := koalabear.GenerateScalars(size)

	var deviceScalars core.DeviceSlice
	scalars.CopyToDevice(&deviceScalars, true)

	koalabear.ToMontgomery(deviceScalars)

	scalarsMontHost := make(core.HostSlice[koalabear.ScalarField], size)

	scalarsMontHost.CopyFromDevice(&deviceScalars)
	suite.NotEqual(scalars, scalarsMontHost)

	koalabear.FromMontgomery(deviceScalars)
...
func (s *ScalarFieldTestSuite) TestScalarField() {
	...
	s.Run("TestKoalabearGenerateScalars", test_helpers.TestWrapper(&s.Suite, testKoalabearGenerateScalars))
	s.Run("TestKoalabearMongtomeryConversion", test_helpers.TestWrapper(&s.Suite, testKoalabearMongtomeryConversion))
}
