...
import (
	...
	"github.com/ingonyama-zk/icicle/v3/wrappers/golang/fields/koalabear/polynomial"
	"github.com/ingonyama-zk/icicle/v3/wrappers/golang/fields/koalabear/vecOps"
	"github.com/stretchr/testify/suite"
)

var one, two, three, four, five koalabear.ScalarField
...
func rand() koalabear.ScalarField {
	return koalabear.GenerateScalars(1)[0]
}

func randomPoly(size int) (f polynomial.DensePolynomial) {
	f.CreateFromCoeffecitients(core.HostSliceFromElements(koalabear.GenerateScalars(size)))
	return f
}

func vecOp(a, b koalabear.ScalarField, op core.VecOps) koalabear.ScalarField {
...
func testPolyCreateFromCoefficients(suite *suite.Suite) {
	scalars := koalabear.GenerateScalars(33)
...
func testPolyEval(suite *suite.Suite) {
	...
	coeffs := core.HostSliceFromElements([]koalabear.ScalarField{five, two, four})
	var f polynomial.DensePolynomial
	f.CreateFromCoeffecitients(coeffs)

	var x koalabear.ScalarField
	x.FromUint32(8)
	...
	var expected koalabear.ScalarField
	suite.Equal(expected.FromUint32(277), fEvaled.(core.HostSlice[koalabear.ScalarField])[0])
}
...
func testPolyMonomials(suite *suite.Suite) {
	var zero koalabear.ScalarField
	var f polynomial.DensePolynomial
	f.CreateFromCoeffecitients(core.HostSliceFromElements([]koalabear.ScalarField{one, zero, two}))
	x := rand()
...
func testPolyReadCoeffs(suite *suite.Suite) {
	var f polynomial.DensePolynomial
	coeffs := core.HostSliceFromElements([]koalabear.ScalarField{one, two, three, four})
	f.CreateFromCoeffecitients(coeffs)
	coeffsCopied := make(core.HostSlice[koalabear.ScalarField], coeffs.Len())
	...
	coeffsHost := make(core.HostSlice[koalabear.ScalarField], coeffs.Len())
...
func testPolyOddEvenSlicing(suite *suite.Suite) {
	...
	x := rand()
	var evenExpected, oddExpected koalabear.ScalarField
	for i := size; i >= 0; i-- {
		if i%2 == 0 {
...
func testPolynomialDivision(suite *suite.Suite) {
	...
	f.CreateFromCoeffecitients(core.HostSliceFromElements(koalabear.GenerateScalars(1 << 4)))
	g.CreateFromCoeffecitients(core.HostSliceFromElements(koalabear.GenerateScalars(1 << 2)))
	...
	x := koalabear.GenerateScalars(1)[0]
	fEval := f.Eval(x)
...
func testDivideByVanishing(suite *suite.Suite) {
	...
	coeffs := core.HostSliceFromElements([]koalabear.ScalarField{minus_one, zero, zero, zero, one}) // x^4-1
...
// 	size := 4
// 	coeffs := koalabear.GenerateScalars(size)
...
// 	hostSlice := make(core.HostSlice[koalabear.ScalarField], size)
// 	hostSlice.CopyFromDevice(fSlice)
// 	suite.Equal(coeffs, hostSlice)

// 	cfg := ntt.GetDefaultNttConfig()
// 	res := make(core.HostSlice[koalabear.ScalarField], size)
