...
var one, two, three, four, five goldilocks.ScalarField
...
func rand() goldilocks.ScalarField {
	return goldilocks.GenerateScalars(1)[0]
}

func randomPoly(size int) (f polynomial.DensePolynomial) {
	f.CreateFromCoeffecitients(core.HostSliceFromElements(goldilocks.GenerateScalars(size)))
	return f
}

func vecOp(a, b goldilocks.ScalarField, op core.VecOps) goldilocks.ScalarField {
...
func testPolyCreateFromCoefficients(suite *suite.Suite) {
	scalars := goldilocks.GenerateScalars(33)
...
func testPolyEval(suite *suite.Suite) {
	...
	coeffs := core.HostSliceFromElements([]goldilocks.ScalarField{five, two, four})
	var f polynomial.DensePolynomial
	f.CreateFromCoeffecitients(coeffs)

	var x goldilocks.ScalarField
	x.FromUint32(8)
	...
	var expected goldilocks.ScalarField
	suite.Equal(expected.FromUint32(277), fEvaled.(core.HostSlice[goldilocks.ScalarField])[0])
}
...
func testPolyMonomials(suite *suite.Suite) {
	var zero goldilocks.ScalarField
	var f polynomial.DensePolynomial
	f.CreateFromCoeffecitients(core.HostSliceFromElements([]goldilocks.ScalarField{one, zero, two}))
	x := rand()
...
func testPolyReadCoeffs(suite *suite.Suite) {
	var f polynomial.DensePolynomial
	coeffs := core.HostSliceFromElements([]goldilocks.ScalarField{one, two, three, four})
	f.CreateFromCoeffecitients(coeffs)
	coeffsCopied := make(core.HostSlice[goldilocks.ScalarField], coeffs.Len())
	...
	coeffsHost := make(core.HostSlice[goldilocks.ScalarField], coeffs.Len())
...
func testPolyOddEvenSlicing(suite *suite.Suite) {
	...
	x := rand()
	var evenExpected, oddExpected goldilocks.ScalarField
	for i := size; i >= 0; i-- {
		if i%2 == 0 {
...
func testPolynomialDivision(suite *suite.Suite) {
	...
	f.CreateFromCoeffecitients(core.HostSliceFromElements(goldilocks.GenerateScalars(1 << 4)))
	g.CreateFromCoeffecitients(core.HostSliceFromElements(goldilocks.GenerateScalars(1 << 2)))
	...
	x := goldilocks.GenerateScalars(1)[0]
	fEval := f.Eval(x)
...
func testDivideByVanishing(suite *suite.Suite) {
	...
	coeffs := core.HostSliceFromElements([]goldilocks.ScalarField{minus_one, zero, zero, zero, one}) // x^4-1
...
// 	size := 4
// 	coeffs := goldilocks.GenerateScalars(size)
...
// 	hostSlice := make(core.HostSlice[goldilocks.ScalarField], size)
// 	hostSlice.CopyFromDevice(fSlice)
// 	suite.Equal(coeffs, hostSlice)

// 	cfg := ntt.GetDefaultNttConfig()
// 	res := make(core.HostSlice[goldilocks.ScalarField], size)
