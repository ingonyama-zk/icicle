...
const (
	SCALAR_LIMBS = goldilocks.SCALAR_LIMBS
)

func testScalarFieldFromLimbs(suite *suite.Suite) {
	emptyField := goldilocks.ScalarField{}
...
func testScalarFieldOne(suite *suite.Suite) {
	var emptyField goldilocks.ScalarField
	emptyField.One()
...
func testGoldilocksGenerateScalars(suite *suite.Suite) {
	const numScalars = 8
	scalars := goldilocks.GenerateScalars(numScalars)

	suite.Implements((*core.HostOrDeviceSlice)(nil), &scalars)

	suite.Equal(numScalars, scalars.Len())
	zeroScalar := goldilocks.ScalarField{}
...
func testScalarFieldArithmetic(suite *suite.Suite) {
	...
	scalarsA := goldilocks.GenerateScalars(size)
	scalarsB := goldilocks.GenerateScalars(size)
	...
	one := scalarA.Mul(&inv)
	expectedOne := goldilocks.GenerateScalars(1)[0]
	expectedOne.One()

	suite.Equal(expectedOne, one)
}

func testGoldilocksMongtomeryConversion(suite *suite.Suite) {
	size := 1 << 20
	scalars := goldilocks.GenerateScalars(size)

	var deviceScalars core.DeviceSlice
	scalars.CopyToDevice(&deviceScalars, true)

	goldilocks.ToMontgomery(deviceScalars)

	scalarsMontHost := make(core.HostSlice[goldilocks.ScalarField], size)

	scalarsMontHost.CopyFromDevice(&deviceScalars)
	suite.NotEqual(scalars, scalarsMontHost)

	goldilocks.FromMontgomery(deviceScalars)
...
func (s *ScalarFieldTestSuite) TestScalarField() {
	...
	s.Run("TestGoldilocksGenerateScalars", test_helpers.TestWrapper(&s.Suite, testGoldilocksGenerateScalars))
	s.Run("TestGoldilocksMongtomeryConversion", test_helpers.TestWrapper(&s.Suite, testGoldilocksMongtomeryConversion))
}
