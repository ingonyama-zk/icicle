...
const (
	SCALAR_LIMBS = m31.SCALAR_LIMBS
)

func testScalarFieldFromLimbs(suite *suite.Suite) {
	emptyField := m31.ScalarField{}
...
func testScalarFieldOne(suite *suite.Suite) {
	var emptyField m31.ScalarField
	emptyField.One()
...
func testM31GenerateScalars(suite *suite.Suite) {
	const numScalars = 8
	scalars := m31.GenerateScalars(numScalars)

	suite.Implements((*core.HostOrDeviceSlice)(nil), &scalars)

	suite.Equal(numScalars, scalars.Len())
	zeroScalar := m31.ScalarField{}
...
func testScalarFieldArithmetic(suite *suite.Suite) {
	...
	scalarsA := m31.GenerateScalars(size)
	scalarsB := m31.GenerateScalars(size)
	...
	one := scalarA.Mul(&inv)
	expectedOne := m31.GenerateScalars(1)[0]
	expectedOne.One()
...
func (s *ScalarFieldTestSuite) TestScalarField() {
	...
	s.Run("TestM31GenerateScalars", test_helpers.TestWrapper(&s.Suite, testM31GenerateScalars))
}
