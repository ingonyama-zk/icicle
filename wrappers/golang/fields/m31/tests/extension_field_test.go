...
const (
	EXTENSION_LIMBS = m31_extension.EXTENSION_LIMBS
)

func testExtensionFieldFromLimbs(suite *suite.Suite) {
	emptyField := m31_extension.ExtensionField{}
...
func testExtensionFieldOne(suite *suite.Suite) {
	var emptyField m31_extension.ExtensionField
	emptyField.One()
...
func testM31_extensionGenerateScalars(suite *suite.Suite) {
	const numScalars = 8
	scalars := m31_extension.GenerateScalars(numScalars)

	suite.Implements((*core.HostOrDeviceSlice)(nil), &scalars)

	suite.Equal(numScalars, scalars.Len())
	zeroScalar := m31_extension.ExtensionField{}
...
func testExtensionFieldArithmetic(suite *suite.Suite) {
	...
	scalarsA := m31_extension.GenerateScalars(size)
	scalarsB := m31_extension.GenerateScalars(size)
	...
	one := scalarA.Mul(&inv)
	expectedOne := m31_extension.GenerateScalars(1)[0]
	expectedOne.One()
...
func (s *ExtensionFieldTestSuite) TestExtensionField() {
	...
	s.Run("TestM31_extensionGenerateScalars", test_helpers.TestWrapper(&s.Suite, testM31_extensionGenerateScalars))
}
