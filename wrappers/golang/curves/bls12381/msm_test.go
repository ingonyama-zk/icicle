...
func testAgainstGnarkCryptoMsmGnarkCryptoTypes(scalarsFr core.HostSlice[fr.Element], pointsFp core.HostSlice[bls12381.G1Affine], out Projective) bool {
...
func convertIcicleAffineToG1Affine(iciclePoints []Affine) []bls12381.G1Affine {
...
func TestMSMGnarkCryptoTypes(t *testing.T) {
	...
	for _, power := range []int{3} {
		...
		e = Msm[fr.Element, bls12381.G1Affine](scalarsHost, pointsHost, &cfg, out)
		assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
		outHost := make(core.HostSlice[Projective], 1)
		outHost.CopyFromDevice(&out)
		out.Free()
