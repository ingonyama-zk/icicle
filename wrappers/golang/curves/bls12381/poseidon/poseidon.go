...
func Create(arity uint32, alpha uint32, fullRoundsHalf uint32, partialRounds uint32, scalars core.HostOrDeviceSlice, mdsMatrix core.HostOrDeviceSlice, nonSparseMatrix core.HostOrDeviceSlice, sparseMatrices core.HostOrDeviceSlice, domainTag bls12_381.ScalarField, ctx *cr.DeviceContext) Poseidon {
	...
	C.bls12_381_poseidon_create_cuda(&poseidon, cArity, cAlpha, cFullRoundsHalf, cPartialRounds, cScalars, cMdsMatrix, cNonSparseMatrix, cSparseMatrices, cDomainTag, cCtx)
...
func Load(arity uint32, ctx *cr.DeviceContext) Poseidon {
	...
	cArity := (C.uint)(arity)
	cCtx := (*C.DeviceContext)(unsafe.Pointer(ctx))
	C.bls12_381_poseidon_load_cuda(&poseidon, cArity, cCtx)
	return Poseidon{handle: poseidon, width: arity + 1}
}
...
func (poseidon *Poseidon) AbsorbMany(inputs core.HostOrDeviceSlice, states core.DeviceSlice, numberOfStates uint32, inputBlockLen uint32, cfg *core.SpongeConfig) core.IcicleError {
	...
	__ret := C.bls12_381_poseidon_absorb_many_cuda(poseidon.handle, cInputs, cStates, cNumberOfStates, cInputBlockLen, cCfg)
...
func (poseidon *Poseidon) SqueezeMany(states core.DeviceSlice, output core.HostOrDeviceSlice, numberOfStates uint32, outputLen uint32, cfg *core.SpongeConfig) core.IcicleError {
	...
	cCfg := (*C.SpongeConfig)(unsafe.Pointer(cfg))
	__ret := C.bls12_381_poseidon_absorb_many_cuda(poseidon.handle, cStates, cOutput, cNumberOfStates, cOutputLen, cCfg)
	err := (cr.CudaError)(__ret)
	return core.FromCudaError(err)
}
...
func (poseidon *Poseidon) Delete(ctx *cr.DeviceContext) core.IcicleError {
	cCtx := (*C.DeviceContext)(unsafe.Pointer(ctx))
	__ret := C.bls12_381_poseidon_delete_cuda(poseidon.handle, cCtx)
	err := (cr.CudaError)(__ret)
	return core.FromCudaError(err)
}
