...
import (
	...
	"github.com/ingonyama-zk/icicle/v3/wrappers/golang/curves/bls12381/poseidon2"
...
func testPoseidon2Hash(s *suite.Suite) {
	batch := 1 << 4
	domainTag := bls12_381.GenerateScalars(1)[0]
	for _, t := range []int{2, 3, 4, 8, 12, 16, 20, 24} {
		...
		for _, domainTag := range []*bls12_381.ScalarField{nil, &domainTag} {
			var inputsSize int
			if domainTag != nil {
			...
			inputs := bls12_381.GenerateScalars(inputsSize)

			// Set device to CPU
			test_helpers.ActivateReferenceDevice()
			outputsRef := make([]bls12_381.ScalarField, batch)
			...
			outputsMain := make([]bls12_381.ScalarField, batch)
...
func testPoseidon2HashTree(s *suite.Suite) {
	...
	leaves := bls12_381.GenerateScalars(int(numElements))
	...
	merkletree.BuildMerkleTree[bls12_381.ScalarField](&merkleTree, core.HostSliceFromElements(leaves), core.GetDefaultMerkleTreeConfig())

	leafIndexToOpen := numElements >> 1
	merkleProof, _ := merkletree.GetMerkleTreeProof[bls12_381.ScalarField](&merkleTree, core.HostSliceFromElements(leaves), uint64(leafIndexToOpen), true /* prunedPath */, core.GetDefaultMerkleTreeConfig())

	// Examples of how to access different pieces of the proof
	_ /*root*/ = merkletree.GetMerkleProofRoot[bls12_381.ScalarField](&merkleProof)
	_ /*path*/ = merkletree.GetMerkleProofPath[bls12_381.ScalarField](&merkleProof)
	_ /*leaf*/, _ /*leafIndex*/ = merkletree.GetMerkleProofLeaf[bls12_381.ScalarField](&merkleProof)
...
func (s *Poseidon2TestSuite) TestPoseidon2() {
	s.Run("TestPoseidon2Hash", test_helpers.test_helpers.TestWrapper(&s.Suite, testPoseidon2Hash))
	s.Run("TestPoseidon2HashSponge", test_helpers.test_helpers.TestWrapper(&s.Suite, testPoseidon2HashSponge))
	s.Run("TestPoseidon2HashTree", test_helpers.test_helpers.TestWrapper(&s.Suite, testPoseidon2HashTree))
}
