...
func testAgainstGnarkCryptoMsmG2(scalars core.HostSlice[icicle_bls12381.ScalarField], points core.HostSlice[G2Affine], out G2Projective) bool {
...
func testAgainstGnarkCryptoMsmG2GnarkCryptoTypes(scalarsFr core.HostSlice[fr.Element], pointsFp core.HostSlice[bls12381.G2Affine], out G2Projective) bool {
...
func convertIcicleG2AffineToG2Affine(iciclePoints []G2Affine) []bls12381.G2Affine {
...
func TestMSMG2(t *testing.T) {
	cfg := icicle_bls12381.GetDefaultMSMConfig()
	cfg.IsAsync = true
	for _, power := range []int{2, 3, 4, 5, 6, 7, 8, 10, 18} {
		size := 1 << power

		scalars := icicle_bls12381.GenerateScalars(size)
		...
		e = G2Msm[icicle_bls12381.ScalarField, G2Affine](scalars, points, &cfg, out)
...
func TestMSMG2GnarkCryptoTypes(t *testing.T) {
	...
	for _, power := range []int{3} {
		...
		e = G2Msm[fr.Element, bls12381.G2Affine](scalarsHost, pointsHost, &cfg, out)
		assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
		outHost := make(core.HostSlice[G2Projective], 1)
...
func TestMSMG2Batch(t *testing.T) {
	...
	for _, power := range []int{10, 16} {
		for _, batchSize := range []int{1, 3, 16} {
			...
			scalars := icicle_bls12381.GenerateScalars(totalSize)
...
func TestPrecomputeBaseG2(t *testing.T) {
	...
	for _, power := range []int{10, 16} {
		for _, batchSize := range []int{1, 3, 16} {
			...
			e = G2Msm[icicle_bls12381.ScalarField, G2Affine](scalars, precomputeOut, &cfg, out)
			assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
			outHost := make(core.HostSlice[G2Projective], batchSize)
			outHost.CopyFromDevice(&out)
			out.Free()
