...
func testAgainstGnarkCryptoMsmg2(scalars core.HostSlice[icicleBls12_377.ScalarField], points core.HostSlice[icicleBls12_377.g2Affine], out icicleBls12_377.g2Projective) bool {
...
func testAgainstGnarkCryptoMsmg2GnarkCryptoTypes(scalarsFr core.HostSlice[fr.Element], pointsFp core.HostSlice[bls12377.G1Affine], out icicleBls12_377.g2Projective) bool {
...
func convertIcicleAffineToG1Affine(iciclePoints []icicleBls12_377.g2Affine) []bls12377.G1Affine {
...
func TestMSMg2(t *testing.T) {
	...
	for _, power := range []int{2, 3, 4, 5, 6, 7, 8, 10, 18} {
		...
		points := icicleBls12_377.g2GenerateAffinePoints(size)

		stream, _ := cr.CreateStream()
		var p icicleBls12_377.g2Projective
		...
		outHost := make(core.HostSlice[icicleBls12_377.g2Projective], 1)
...
func TestMSMg2Batch(t *testing.T) {
	...
	for _, power := range []int{10, 16} {
		for _, batchSize := range []int{1, 3, 16} {
			...
			points := icicleBls12_377.g2GenerateAffinePoints(totalSize)
			...
			e = g2Msm(scalars, points, &cfg, out)
			assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
			outHost := make(core.HostSlice[icicleBls12_377.g2Projective], batchSize)
			outHost.CopyFromDevice(&out)
			out.Free()
			// Check with gnark-crypto
