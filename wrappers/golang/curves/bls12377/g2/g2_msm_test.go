...
func testAgainstGnarkCryptoMsmG2GnarkCryptoTypes(scalarsFr core.HostSlice[fr.Element], pointsFp core.HostSlice[bls12377.G2Affine], out G2Projective) bool {
...
func convertIcicleG2AffineToG2Affine(iciclePoints []G2Affine) []bls12377.G2Affine {
...
func TestMSMG2(t *testing.T) {
	...
	for _, power := range []int{2, 3, 4, 5, 6, 7, 8, 10, 18} {
		...
		e = G2Msm[icicle_bls12377.ScalarField, G2Affine](scalars, points, &cfg, out)
...
func TestMSMG2GnarkCryptoTypes(t *testing.T) {
	...
	for _, power := range []int{3} {
		...
		e = G2Msm[fr.Element, bls12377.G2Affine](scalarsHost, pointsHost, &cfg, out)
		assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
		outHost := make(core.HostSlice[G2Projective], 1)
...
func TestPrecomputeBaseG2(t *testing.T) {
	...
	for _, power := range []int{10, 16} {
		for _, batchSize := range []int{1, 3, 16} {
			...
			e = G2Msm[icicle_bls12377.ScalarField, G2Affine](scalars, precomputeOut, &cfg, out)
			assert.Equal(t, e, cr.CudaSuccess, "Msm failed")
			outHost := make(core.HostSlice[G2Projective], batchSize)
			outHost.CopyFromDevice(&out)
			out.Free()
