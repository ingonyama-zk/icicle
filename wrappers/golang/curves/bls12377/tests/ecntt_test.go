...
func testECNtt(suite *suite.Suite) {
	...
	for _, size := range []int{4, 9, 11} {
		for _, dir := range []core.NTTDir{core.KForward, core.KInverse} {
			...
			points := bls12_377.GenerateProjectivePoints(testSize)
			...
			outputMain := make(core.HostSlice[bls12_377.Projective], testSize)
			...
			outputMainOrig := make(core.HostSlice[bls12_377.Projective], testSize)
