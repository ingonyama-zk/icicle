...
pub(crate) mod tests {
  ...
  use icicle_cuda_runtime::memory::{HostSlice, DeviceVec};
  use std::iter::zip;
  ...
  fn test_fold_line() {
    ...
    d_eval.copy_from_host(eval).unwrap();
    ...
    d_domain_elements.copy_from_host(domain_elements).unwrap();
  ...
  fn test_fold_circle_to_line() {
    ...
    d_circle_eval.copy_from_host(circle_eval).unwrap();
    
    let domain_raw: [u32; DEGREE/2] = [1774253895, 373229752, 1309288441, 838195206, 262191051, 1885292596, 408478793, 1739004854, 212443077, 1935040570, 1941424532, 206059115, 883753057, 1263730590, 350742286, 1796741361, 404685994, 1742797653, 7144319, 2140339328, 68458636, 2079025011, 2137679949, 9803698, 228509164, 1918974483, 2132953617, 14530030, 134155457, 2013328190, 1108537731, 1038945916];
    let domain_as_scalar = domain_raw
      .into_iter()
      .map(
        |val: u32| ScalarField::from_u32(val)
    ...
    let expected_folded_evals_raw: [u32; DEGREE/2] = [1188788264, 1195916566, 953551618, 505128535, 403386644, 1619126710, 988135024, 1735901259, 1587281171, 907165282, 799778920, 1532707002, 348262725, 267076231, 902054839, 98124803, 1953436582, 267778518, 632724299, 460151826, 2139528518, 1378487361, 1709496698, 48330818, 1343585282, 1852541250, 727719914, 1964971391, 1423101288, 2099768709, 274685472, 1051044961];
    let expected_folded_evals_extension = expected_folded_evals_raw
      .into_iter()
      .map(
        |val: u32| ExtensionField::from_u32(val)
