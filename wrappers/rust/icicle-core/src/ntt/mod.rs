    ...
    (
      ...
      $field_config:ident
        ...
        fn test_ntt() {
            ...
            $field_config::initialize_domain(
                $field::from_ark(Fr::get_root_of_unity(test_size as u64).unwrap()),
                &ctx,
            )
            .unwrap();
            let ark_domain = GeneralEvaluationDomain::<Fr>::new(test_size).unwrap();

            let scalars: Vec<$field> = generate_random_scalars(test_size);

            let config = $field_config::get_default_ntt_config();
            let mut ntt_result = vec![$field::zero(); test_size];
            $field_config::ntt(&scalars, false, &config, &mut ntt_result).unwrap();
            ...
            let mut intt_result = vec![$field::zero(); test_size];
            $field_config::ntt(&ntt_result, true, &config, &mut intt_result).unwrap();
        ...
        fn test_ntt_coset_from_subgroup() {
            ...
            // two roughly analogous calls for icicle and arkworks. one difference is that icicle call creates
            // domain for all NTTs of size <= `test_size`. also for icicle domain is a hidden static object
            $field_config::initialize_domain($field::from_ark(test_size_rou), &ctx).unwrap();
            ...
            let mut scalars: Vec<$field> = generate_random_scalars(small_size);

            let mut config = $field_config::get_default_ntt_config();
            config.ordering = Ordering::kNR;
            let mut ntt_result = vec![$field::zero(); test_size];
            $field_config::ntt(&scalars, false, &config, &mut ntt_result[..small_size]).unwrap();
            assert_ne!(ntt_result[..small_size], scalars);
            config.coset_gen = $field::from_ark(test_size_rou);
            $field_config::ntt(&scalars, false, &config, &mut ntt_result[small_size..]).unwrap();
            let mut ntt_large_result = vec![$field::zero(); test_size];
            // back to non-coset NTT
            config.coset_gen = $field::one();
            scalars.resize(test_size, $field::zero());
            $field_config::ntt(&scalars, false, &config, &mut ntt_large_result).unwrap();
            ...
            let mut intt_result = vec![$field::zero(); small_size];
            $field_config::ntt(&ntt_result[small_size..], true, &config, &mut intt_result).unwrap();
