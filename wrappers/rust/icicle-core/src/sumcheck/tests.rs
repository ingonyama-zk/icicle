    ...
    P: ReturningValueProgram,
{
    ...
    let challenge_vector = sumcheck.get_challenge_vector().unwrap();
    assert!(challenge_vector.is_empty(), "Challenge vector should be empty before proving");
    ...
    let claimed_sum = mle_poly_a.iter()
        .zip(mle_poly_b.iter())
        .zip(mle_poly_c.iter())
    ...
    for (i, &challenge) in challenge_vector.iter().enumerate().skip(1) {
        assert!(
