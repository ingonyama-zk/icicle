...
public:
  ...
  void setup()
  {
    ...
    const int m = witness_size;
    const int l = 1 + nof_outputs;         // public part of the witness
    const int n = max(m, nof_constraints); // TODO Yuval: should be the polynomials degree but it's smaller than the
                                           // witness size so doesn't make sense ??
    ...
    // {tau^i(t(tau) / delta}
    pk.g1.vanishing_poly_points.reserve(n - 2);
    ...
    auto x = toxic_waste.tau;
    pk.g1.vanishing_poly_points.push_back(projective_t::to_affine(t(toxic_waste.tau) * toxic_waste.delta_inv * G1));
    for (int i = 0; i < n - 2; ++i) {
    ...
    pk.g2.betta = g2_projective_t::to_affine(toxic_waste.beta * G2);
  ...
  G16proof prove(std::vector<scalar_t>& witness)
  {
    ...
    // compute [C]1
    {
      ...
      CHK_STICKY(msm::_MSM(
        witness.data() + l, pk.g1.private_witness_points.data(), m - l, msm_config, &private_inputs_commited));
