...
//       ITERS);
//   }
//   run(IcicleTestBase::main_device(), out_main.get(), VERBOSE /*=measure*/, vector_sum<scalar_t>, "vector sum",
//   ITERS); ASSERT_EQ(0, memcmp(out_main.get(), out_ref.get(), batch_size * sizeof(scalar_t)));
...
//                   2, 9); // cpu implementation for out of place transpose also supports sizes which are not powers of
//                   2
...
//   run(
//     IcicleTestBase::main_device(), (is_in_place ? h_inout.get() : h_out_main.get()), VERBOSE /*=measure*/,
//     "transpose", ITERS);
...
//   }
//   run(
//     IcicleTestBase::main_device(), (is_in_place ? in_a.get() : out_main.get()), VERBOSE /*=measure*/, "bit-reverse",
//     1);
...
//   auto verifier_sumcheck = create_sumcheck<scalar_t>();
//   bool verification_pass = false;
//   ICICLE_CHECK(verifier_sumcheck.verify(sumcheck_proof, claimed_sum, std::move(transcript_config),
//   verification_pass));
