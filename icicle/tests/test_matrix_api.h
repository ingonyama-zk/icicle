...
TEST_F(MatrixTestBase, MatrixMultiplicationBatched)
{
  ...
  for (size_t i = 0; i < batch_size; i++) {
    for (size_t j = 0; j < matrix_size * matrix_size; j++) {
      ASSERT_EQ(direct_output[i][j], icicle_output[i][j]);
    }
  }
}
...
TEST_F(MatrixTestBase, MatrixMultiplicationNonSquare)
{
  ...
  // Compute result using icicle CPU backend
  auto cfg = VecOpsConfig{};
  cfg.is_a_on_device = true;
  cfg.is_b_on_device = true;
  cfg.is_result_on_device = true;
  ...
  for (size_t i = 0; i < N * M; i++) {
    ASSERT_EQ(direct_output[i], icicle_output[i]);
  }
}
...
TEST_F(MatrixTestBase, MatrixMultiplicationSanityChecks)
{
  ...
  for (size_t i = 0; i < matrix_size * matrix_size; i++) {
    ASSERT_EQ(random_matrix[i], result[i]);
  }
  ...
  for (size_t i = 0; i < matrix_size * matrix_size; i++) {
    ASSERT_EQ(expected_zero[i], result[i]);
  }
  ...
  for (size_t i = 0; i < matrix_size; i++) {
    ASSERT_EQ(expected_vector_result[i], vector_result[i]);
  }
  ...
  for (size_t i = 0; i < matrix_size; i++) {
    for (size_t j = 0; j < matrix_size; j++) {
      ASSERT_EQ(vector_a[i] * vector_b[j], outer_product_result[i * matrix_size + j]);
    }
  }
  ...
  for (size_t i = 0; i < matrix_size; i++) {
    ASSERT_EQ(expected_vector_matrix_result[i], vector_matrix_result[i]);
  }
}
