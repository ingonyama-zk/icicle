...
TEST_F(MatrixTestBase, MatrixMultiplicationBatched)
{
  ...
  for (size_t i = 0; i < batch_size; i++) {
    multiply_matrices(batch_a[i], single_b, direct_output[i], 
  ...
  for (const auto& device : s_registered_devices) {
    ...
    for (size_t i = 0; i < batch_size; i++) {
      ICICLE_CHECK(matrix_mult(batch_a[i].data(), matrix_size, matrix_size,
                              single_b.data(), matrix_size, matrix_size,
                              cfg, icicle_output[i].data()));
    }

    // Compare results for each matrix in the batch
    for (size_t i = 0; i < batch_size; i++) {
      ASSERT_EQ(0, std::memcmp(direct_output[i].data(), icicle_output[i].data(), direct_output[i].size() * sizeof(scalar_t)));
    }
  }
}
...
TEST_F(MatrixTestBase, MatrixMultiplicationSanityChecks)
{
  ...
  for (const auto& device : s_registered_devices) {
    ...
    // A * I = A
    ICICLE_CHECK(matrix_mult(random_matrix.data(), matrix_size, matrix_size,
                            identity.data(), matrix_size, matrix_size,
                            cfg, result.data()));
    
    ASSERT_EQ(0, std::memcmp(random_matrix.data(), result.data(), random_matrix.size() * sizeof(scalar_t)));

    // I * A = A
    ICICLE_CHECK(matrix_mult(identity.data(), matrix_size, matrix_size,
                            random_matrix.data(), matrix_size, matrix_size,
                            cfg, result.data()));
    ...
    // A * 0 = 0
    ICICLE_CHECK(matrix_mult(random_matrix.data(), matrix_size, matrix_size,
                            zero_matrix.data(), matrix_size, matrix_size,
                            cfg, result.data()));
    
    ASSERT_EQ(0, std::memcmp(expected_zero.data(), result.data(), expected_zero.size() * sizeof(scalar_t)));

    // 0 * A = 0
    ICICLE_CHECK(matrix_mult(zero_matrix.data(), matrix_size, matrix_size,
                            random_matrix.data(), matrix_size, matrix_size,
                            cfg, result.data()));
    ...
    ICICLE_CHECK(matrix_mult(single_a.data(), 1, 1,
                            single_b.data(), 1, 1,
                            cfg, single_result.data()));
    ...
    // Matrix * Vector
    ICICLE_CHECK(matrix_mult(random_matrix.data(), matrix_size, matrix_size,
                            vector.data(), matrix_size, 1,
                            cfg, vector_result.data()));

    // Verify result dimensions and values
    std::vector<scalar_t> expected_vector_result(matrix_size);
    multiply_matrices(random_matrix, vector, expected_vector_result, 
                     matrix_size, matrix_size, 1);
    
    ASSERT_EQ(0, std::memcmp(expected_vector_result.data(), vector_result.data(), expected_vector_result.size() * sizeof(scalar_t)));
    ...
    ICICLE_CHECK(matrix_mult(vector_a.data(), matrix_size, 1,
                            vector_b.data(), 1, matrix_size,
                            cfg, outer_product_result.data()));
    ...
    ASSERT_EQ(0, std::memcmp(expected_outer_product.data(), outer_product_result.data(), expected_outer_product.size() * sizeof(scalar_t)));

    // Test 6: Vector-Matrix multiplication
    std::vector<scalar_t> vector_matrix_result(matrix_size);
    
    // Vector^T * Matrix
    ICICLE_CHECK(matrix_mult(vector.data(), 1, matrix_size,
                            random_matrix.data(), matrix_size, matrix_size,
                            cfg, vector_matrix_result.data()));

    // Verify result dimensions and values
    std::vector<scalar_t> expected_vector_matrix_result(matrix_size);
    multiply_matrices(vector, random_matrix, expected_vector_matrix_result, 
                     1, matrix_size, matrix_size);
    
    ASSERT_EQ(0, std::memcmp(expected_vector_matrix_result.data(), vector_matrix_result.data(), expected_vector_matrix_result.size() * sizeof(scalar_t)));
  }
}
...
TEST_F(MatrixTestBase, MatrixMultiplicationDimensionMismatch)
{
  ...
  for (const auto& device : s_registered_devices) {
    ...
    auto error = matrix_mult(matrix_a.data(), matrix_size, matrix_size,
                            matrix_b.data(), matrix_size + 1, matrix_size,
                            cfg, result.data());
...
TEST_F(MatrixTestBase, MatrixMultiplicationBatchedDimensionMismatch)
{
  ...
  for (const auto& device : s_registered_devices) {
    ...
    for (size_t i = 0; i < batch_size; i++) {
      auto error = matrix_mult(batch_a[i].data(), matrix_size, matrix_size,
                             single_b.data(), matrix_size + 1, matrix_size,
                             cfg, batch_result[i].data());
      ASSERT_NE(error, eIcicleError::SUCCESS);
    }
  }
}
...
TEST_F(MatrixTestBase, MatrixMultiplicationNullInputs)
{
  ...
  for (const auto& device : s_registered_devices) {
    ...
    // Test null A matrix
    auto error = matrix_mult(static_cast<const scalar_t*>(nullptr), matrix_size, matrix_size,
                           static_cast<const scalar_t*>(valid_matrix.data()), matrix_size, matrix_size,
                           cfg, result.data());
    ASSERT_NE(error, eIcicleError::SUCCESS);

    // Test null B matrix
    error = matrix_mult(static_cast<const scalar_t*>(valid_matrix.data()), matrix_size, matrix_size,
    ...
    error = matrix_mult(static_cast<const scalar_t*>(valid_matrix.data()), matrix_size, matrix_size,
                       static_cast<const scalar_t*>(valid_matrix.data()), matrix_size, matrix_size,
                       cfg, static_cast<scalar_t*>(nullptr));
    ASSERT_NE(error, eIcicleError::SUCCESS);
  }
}
...
TEST_F(MatrixTestBase, MatrixMultiplicationZeroDimensions)
{
  ...
  for (const auto& device : s_registered_devices) {
    ...
    // Test zero rows
    auto error = matrix_mult(valid_matrix.data(), 0, matrix_size,
                           valid_matrix.data(), matrix_size, matrix_size,
                           cfg, result.data());
    ASSERT_NE(error, eIcicleError::SUCCESS);

    // Test zero columns
    error = matrix_mult(valid_matrix.data(), matrix_size, 0,
                       valid_matrix.data(), matrix_size, matrix_size,
                       cfg, result.data());
    ASSERT_NE(error, eIcicleError::SUCCESS);
  }
