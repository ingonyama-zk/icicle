...
def get_mont_const(p, bit_size):
    return (2 ** bit_size) % p

def to_mont(input, mont_r, p):
    return (input * mont_r) % p
...
# Preparation to convert to montgomery format.
bit_size = len(bin(p))-2
mont_r = get_mont_const(p, bit_size)
print(f'field = {FIELD_NAME}, p = {hex(p)}, mont_r = {mont_r}, bit_size = {bit_size}') 
...
for rc_idx in range(R_F_FIXED_HALF * t):
  FILE_cpp.write(f'  "{hex(to_mont(rounds_constants[rc_idx], mont_r, p))}",\n')       # '"' is needed because we need to write out string values.
...
for row in range(MATRIX_FULL.nrows()):
  for col in range(MATRIX_FULL.ncols()):
    ...
    FILE_cpp.write(f' "{hex(to_mont(MATRIX_FULL[row,col], mont_r, p))}"')
...
for i in range(len(MATRIX_PARTIAL_DIAGONAL)):
  ...
  FILE_cpp.write(f' "{hex(to_mont(MATRIX_PARTIAL_DIAGONAL[i], mont_r, p))}"')
