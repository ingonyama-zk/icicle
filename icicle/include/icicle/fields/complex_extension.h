...
public:
  ...
  static HOST_DEVICE_INLINE FF mul_weierstrass_3b_real(const FF& xs)
  {
    ...
    if constexpr (Gen::is_b_u32_g2_re) {
      ...
      return FF{Gen::weierstrass_3b_g2_re} * xs;
    }
  }

  template <typename Gen>
  static HOST_DEVICE_INLINE FF mul_weierstrass_3b_imag(const FF& xs)
  {
    FF r = {};
    if constexpr (Gen::is_b_u32_g2_im) {
      r = FF::template mul_unsigned<FF{Gen::weierstrass_3b_g2_im}.limbs_storage.limbs[0], FF>(xs);
      ...
      return FF{Gen::weierstrass_3b_g2_im} * xs;
    }
  }

  template <typename Gen>
  static HOST_DEVICE_INLINE ComplexExtensionField mul_weierstrass_3b(const ComplexExtensionField& xs)
  {
    const FF xs_real = xs.real;
    const FF xs_imaginary = xs.imaginary;
    FF real_prod = mul_weierstrass_3b_real<Gen>(xs_real);
    FF imaginary_prod = mul_weierstrass_3b_imag<Gen>(xs_imaginary);
    FF re_im = mul_weierstrass_3b_real<Gen>(xs_imaginary);
    FF im_re = mul_weierstrass_3b_imag<Gen>(xs_real);
