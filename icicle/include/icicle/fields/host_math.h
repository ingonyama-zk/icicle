...
namespace host_math {
  ...
  static HOST_INLINE void sos_mont_reduction_32(uint32_t* t, const uint32_t* n, const uint32_t n_tag0, uint32_t* r)
  {
    ...
    for (int i = 0; i < s; i++) {
      uint32_t c = 0;
      uint32_t m = t[i] * n_tag0;

      for (int j = 0; j < s; j++) {
        t[i + j] = madc_cc(m, n[j], t[i + j], c);
      }
      // Propagate the carry to the remaining sublimbs
      for (int carry_idx = s + i; carry_idx < 2 * s; carry_idx++) {
        if (c == 0) { break; }
        t[carry_idx - i] = add_cc(t[carry_idx], c, c);
      }
    }
    get_half_32<NLIMBS>(t, r);
  }
  ...
  static HOST_INLINE void sos_mont_reduction_64(uint64_t* t, const uint64_t* n, const uint64_t n_tag0, uint64_t* r)
  {
    ...
    for (int i = 0; i < s; i++) {
      uint64_t c = 0;
      uint64_t m = t[i] * n_tag0;

      for (int j = 0; j < s; j++) {
        t[i + j] = madc_cc_64(m, n[j], t[i + j], c);
      }
      // Propagate the carry to the remaining sublimbs
      for (int carry_idx = s + i; carry_idx < 2 * s; carry_idx++) {
        if (c == 0) { break; }
        t[carry_idx] = add_cc(t[carry_idx], c, c);
      }
    }
    get_half_64<NLIMBS>(t, r);
  }
  ...
  reduce_mont(storage<2 * NLIMBS>& t, const storage<NLIMBS>& n, const storage<NLIMBS>& n_tag, storage<NLIMBS>& r)
  {
    ...
    if constexpr (USE_32) {
      sos_mont_reduction_32<NLIMBS>(t.limbs, n.limbs, n_tag.limbs[0], r.limbs);
      return;
    } else if constexpr (NLIMBS == 1) {
      sos_mont_reduction_32<NLIMBS>(t.limbs, n.limbs, n_tag.limbs[0], r.limbs);
      return;
    } else {
      sos_mont_reduction_64<NLIMBS>(t.limbs64, n.limbs64, n_tag.limbs64[0], r.limbs64);
    }
  }
  ...
  template <unsigned NLIMBS>
  static constexpr HOST_INLINE void sqr_mont(
    const storage<NLIMBS>& xs,
    const storage<NLIMBS>& mont_inv_modulus,
    const storage<NLIMBS>& modulus,
    storage<NLIMBS>& rs)
  {
