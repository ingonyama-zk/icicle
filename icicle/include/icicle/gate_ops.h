...
namespace icicle {
  ...
  struct LookupConfig {
    ...
    bool is_permuted_input_coset_on_device; /** True if `permuted_input_coset` is on the device. Default: false. */
    bool is_permuted_table_coset_on_device; /** True if `permuted_table_coset` is on the device. Default: false. */
    bool is_product_coset_on_device;        /** True if `product_coset` columns are on the device. Default: false. */
    bool is_l_on_device; /** True if `l0`, `l_last` and `l_active_row` columns are on the device. Default: false. */
    bool is_previous_values_on_device; /** True if `previous_value` array is on the device. Default: false. */
    bool is_result_on_device;          /** True to keep results on device. Default: false. */
    bool is_async;
  };
  ...
  struct LookupData {
    ...
    const T* permuted_input_coset;
    uint32_t num_permuted_input_coset;
    const T* permuted_table_coset;
    uint32_t num_permuted_table_coset;
    const T* product_coset;
    uint32_t num_product_coset;
    const T* l0;
    uint32_t num_l0;
    const T* l_last;
    uint32_t num_l_last;
   ...
   */
  template <typename T>
  eIcicleError lookups_constraint(const LookupData<T>& gate_data, const LookupConfig& config, T* results);
