...
namespace icicle {
  ...
  public:
      ...
      bool& valid /* OUT */) const
    {
      ...
      std::vector<size_t> queries_indicies = transcript.rand_queries_indicies(fri_config.nof_queries, final_poly_size, 1 << log_input_size);
      eIcicleError err = verify_queries(fri_proof, fri_config.nof_queries, queries_indicies, alpha_values, queries_valid);
  ...
  private:
     ...
     */
    void update_transcript_and_generate_alphas_from_proof(FriProof<F>& fri_proof, FriTranscript<F>& transcript, const size_t nof_fri_rounds, std::vector<F>& alpha_values) const{
     ...
     */
    void check_pow_nonce_and_set_to_transcript(FriProof<F>& fri_proof, FriTranscript<F>& transcript, const FriConfig& fri_config, bool& pow_valid) const{
    ...
    /**
     * @brief Checks if the leaf index from the proof matches the expected index computed based on the transcript random generation.
     ...
     */
    bool leaf_index_consistency_check(const uint64_t leaf_index, const uint64_t leaf_index_sym, const size_t query, const size_t round_idx, const uint32_t log_input_size) const {
     ...
     */
    bool collinearity_check(FriProof<F>& fri_proof, const std::byte* leaf_data, const std::byte* leaf_data_sym, const size_t query_idx, const size_t query, const size_t round_idx, std::vector<F>& alpha_values, const uint32_t log_input_size, const S primitive_root_inv) const {
     ...
     */
    bool verify_merkle_proofs_for_query(const MerkleTree& current_round_tree,  MerkleProof& proof_ref, MerkleProof& proof_ref_sym) const {
    ...
    eIcicleError verify_queries(FriProof<F>& fri_proof, const size_t nof_queries, std::vector<size_t>& queries_indicies, std::vector<F>& alpha_values, bool& queries_valid) const{
      const uint32_t log_input_size = fri_proof.get_nof_fri_rounds() + static_cast<uint32_t>(std::log2(fri_proof.get_final_poly_size()));
      ...
      for (size_t query_idx = 0; query_idx < nof_queries; query_idx++) {
        ...
        for (size_t round_idx = 0; round_idx < fri_proof.get_nof_fri_rounds(); ++round_idx) {
          ...
          if (!collinearity_check(fri_proof, leaf_data, leaf_data_sym, query_idx, query, round_idx, alpha_values, log_input_size, primitive_root_inv)){
            return eIcicleError::SUCCESS; // return with queries_valid = false
          }
