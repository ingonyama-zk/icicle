  ...
  public:
    ...
    FriTranscriptConfig(FriTranscriptConfig&& other) noexcept
        : m_hasher(std::move(other.m_hasher)), m_domain_separator_label(std::move(other.m_domain_separator_label)),
          ...
          m_commit_phase_label(std::move(other.m_commit_phase_label)), m_nonce_label(std::move(other.m_nonce_label)),
          m_public(std::move(other.m_public)), m_seed_rng(other.m_seed_rng)
    {
    }
    ...
    const std::vector<std::byte>& get_commit_phase_label() const { return m_commit_phase_label; }

    const std::vector<std::byte>& get_nonce_label() const { return m_nonce_label; }

    const std::vector<std::byte>& get_public_state() const { return m_public; }
