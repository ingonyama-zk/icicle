...
namespace icicle {
  ...
  public:
    ...
    //  */
    // std::tuple<const std::byte*, std::size_t, uint64_t> get_leaf(const size_t query_idx, const size_t round_idx) const
    // {
     ...
     */
    size_t get_nof_fri_rounds() const
    {
      return m_query_proofs[0].size();
    }
     ...
     */
    size_t get_final_poly_size() const
    {
      return m_final_poly_size;
    }
  ...
  private:
    std::vector<std::vector<MerkleProof>> m_query_proofs; // Matrix of Merkle proofs [query][round] - contains path, root, leaf. for each query, we have 2 proofs (for the leaf in [2*query] and its symmetric in [2*query+1])
