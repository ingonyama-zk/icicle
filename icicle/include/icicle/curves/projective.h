...
public:
  ...
  template <bool IS_3B = false>
  static HOST_DEVICE_INLINE FF mul_weierstrass_b(const FF& xs)
  {
    constexpr FF b_mult = []() {
      FF b_mult = FF{Gen::weierstrass_b};
      if constexpr (!IS_3B) return b_mult;
      typename FF::ff_storage temp = {};
      auto modulus = FF::template get_modulus<>();
      host_math::template add_sub_limbs<FF::TLC, false, false, true>(
        b_mult.limbs_storage, b_mult.limbs_storage, b_mult.limbs_storage);
      b_mult.limbs_storage =
        host_math::template add_sub_limbs<FF::TLC, true, true, true>(b_mult.limbs_storage, modulus, temp)
          ? b_mult.limbs_storage
          : temp;
      host_math::template add_sub_limbs<FF::TLC, false, false, true>(
        b_mult.limbs_storage, FF{Gen::weierstrass_b}.limbs_storage, b_mult.limbs_storage);
      b_mult.limbs_storage =
        host_math::template add_sub_limbs<FF::TLC, true, true, true>(b_mult.limbs_storage, modulus, temp)
          ? b_mult.limbs_storage
          : temp;
    ...
    if constexpr (Gen::is_b_u32) { // assumes that 3b is also u32
      auto r = FF::template mul_unsigned<b_mult.limbs_storage.limbs[0], FF>(xs);
  ...
  static HOST_DEVICE Projective dbl(const Projective& point)
  {
    ...
    t2 = mul_weierstrass_b<true>(t2); // 7. t2 ← b3 · t2
  ...
  friend HOST_DEVICE Projective operator+(Projective p1, const Projective& p2)
  {
    ...
    const FF t20 = mul_weierstrass_b<true>(t02); // t20 ← b3 · t02    < 2
    const FF t21 = t01 + t20;                    // t21 ← t01 + t20   < 2
    const FF t22 = t01 - t20;                    // t22 ← t01 − t20   < 2
    const FF t23 = mul_weierstrass_b<true>(t17); // t23 ← b3 · t17    < 2
  ...
  static HOST_DEVICE_INLINE bool is_on_curve(const Projective& point)
  {
    ...
    bool eq_holds =
      (mul_weierstrass_b<>(FF::sqr(point.z) * point.z) + FF::sqr(point.x) * point.x == point.z * FF::sqr(point.y));
