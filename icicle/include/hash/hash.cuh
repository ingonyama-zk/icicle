...
public:
    ...
    bool is_async)
  {
    ...
    S* d_states = nullptr;
    if (!states_on_device) {
      CHK_IF_RETURN(cudaMallocAsync(&d_states, number_of_states * this->width * sizeof(S), ctx.stream));
      CHK_IF_RETURN(cudaMemcpyAsync(
        d_states, states, number_of_states * this->width * sizeof(S), cudaMemcpyHostToDevice, ctx.stream));
    }
    ...
    if (!states_on_device) {
      CHK_IF_RETURN(cudaMemcpyAsync(
        output, d_states, number_of_states * this->width * sizeof(S), cudaMemcpyDeviceToHost, ctx.stream));
...
public:
    ...
    bool is_async) const
  {
    ...
    if (need_allocate_perm_output) {
      CHK_IF_RETURN(cudaMallocAsync(&perm_output, number_of_states * this->width * sizeof(S), ctx.stream))
    }

    // Copy states from host if they are on host
    if (!states_on_device) {
      CHK_IF_RETURN(cudaMemcpyAsync(
        perm_output, states, number_of_states * this->width * sizeof(S), cudaMemcpyHostToDevice, ctx.stream));
    }
    ...
    if (output != perm_output) {
      CHK_IF_RETURN(cudaMemcpyAsync(
        output, perm_output, number_of_states * sizeof(S),
...
class SpongeHasher : Permutation<Image>
{
  ...
  virtual cudaError_t squeeze_many(Image* states, Image* output, unsigned int number_of_states, SpongeConfig& cfg)
  {
