...
void print_array_of_ec_points(affine_t* a,uint32_t n){
  printf("-------------------\n");
  for (int i = 0; i< n; i++){
    std::cout<< i << " x ";
    for (int j = 0; j< 12; j++) {
      std::cout<<a[i].x.limbs_storage.limbs[j] << " ";
    }
    std::cout << " "<<std::endl;
    std::cout<< i << " y ";
    for (int j = 0; j< 12; j++) {
      std::cout<<a[i].y.limbs_storage.limbs[j] << " ";
    }
    std::cout << " "<<std::endl;
  }
}

void print_array_of_ec_points_proj(projective_t* a,uint32_t n){
  printf("-------------------\n");
  for (int i = 0; i< n; i++){
    std::cout<< i << " x ";
    for (int j = 0; j< 12; j++) {
    ...
    std::cout<< i << " z ";
    for (int j = 0; j< 12; j++) {
      std::cout<<a[i].z.limbs_storage.limbs[j] << " ";
    }
    std::cout << " "<<std::endl;
  }
}

void test_ecntt_end2end(int size) {
  printf("test scalar ntt, size: %d ", size);
  uint32_t n = size;
  projective_t * inputs = new projective_t[n];
  generate_projective_t_array(inputs, n);
  affine_t * i = to_affine(inputs, n);
  // print_array_of_ec_points(i, n);
  int res1 = ecntt_end2end(inputs, n, false);
  // print_array_of_ec_points_proj(inputs, n); 
  int res2 = ecntt_end2end(inputs, n, true);
  affine_t * i2 = to_affine(inputs, n);
  // print_array_of_ec_points(i2, n);
}
...
int main(int argc, char * argv[]) {
  ...
  // end2end test
  test_ecntt_end2end(4);
  test_ecntt_end2end(256);
  test_ecntt_end2end(512);
}