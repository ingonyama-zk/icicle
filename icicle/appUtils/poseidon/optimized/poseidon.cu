...
namespace poseidon {
  ...
  __device__ S vecs_mul_matrix(S element, S* matrix, int element_number, int vec_number, int size, S* shared_states)
  {
    ...
    for (int i = 1; i < size; i++) {
      element_wide =
        element_wide + S::mul_wide(shared_states[vec_number * size + i], matrix[i * size + element_number]);
    }
    __syncthreads();
    ...
    const PoseidonConfiguration<S> config)
  {
    ...
    if (!skip_rc) { element = element + config.round_constants[rc_offset + element_number]; }
    ...
    S* states, size_t number_of_states, size_t rc_offset, bool first_half, const PoseidonConfiguration<S> config)
  {
    ...
    for (int i = 0; i < config.full_rounds_half; i++) {
      states[idx] = full_round(
        states[idx], rc_offset, local_state_number, element_number, !first_half || (i < (config.full_rounds_half - 1)),
