...
namespace vec_ops {
   ...
   * @return `cudaSuccess` if the execution was successful and an error code otherwise.
   */
  extern "C" cudaError_t CONCAT_EXPAND(FIELD, c_extension_accumulate_cuda)(
    c_extension_t* vec_a, c_extension_t* vec_b, int n, VecOpsConfig& config)
  {
  ...
  extern "C" cudaError_t CONCAT_EXPAND(FIELD, c_extension_fold_cuda)(
    ...
    scalar_t* vec_a, c_extension_t* vec_b, int n, VecOpsConfig& config, c_extension_t* result, c_extension_t* d_interim_results)
  {
    // return fold<c_extension_t, scalar_t>(vec_a, vec_b, n, config, result);
    return fold<c_extension_t, scalar_t>(vec_a, vec_b, n, config, result, d_interim_results);
  }
  ...
  CONCAT_EXPAND(FIELD, c_extension_inv_cuda)(c_extension_t* vec_a, int n, VecOpsConfig& config, c_extension_t* result)
  {
    return inv<c_extension_t>(vec_a, n, config, result);
  }
} // namespace vec_ops