...
template <>
eIcicleError ring_random_sampling(uint64_t size, bool fast_mode, const std::byte* seed, uint64_t seed_len, const VecOpsConfig& config, field_t* output)
{
  return RandomSamplingRingDispatcher::execute(size, fast_mode, seed, seed_len, config, output);
}
...
template <>
eIcicleError ring_random_sampling(uint64_t size, bool fast_mode, const std::byte* seed, uint64_t seed_len, const VecOpsConfig& config, Rq* output)
{
  return RandomSamplingRingRqDispatcher::execute(size, fast_mode, seed, seed_len, config, output);
}
} // namespace icicle