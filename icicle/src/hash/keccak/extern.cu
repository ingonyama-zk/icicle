...
namespace keccak {
  ...
  {
    SpongeConfig cfg{
      config.ctx,     config.are_inputs_on_device, config.are_outputs_on_device, input_block_size, 4, 136, 17, 0,
      config.is_async};
  ...
  {
    SpongeConfig cfg{
      config.ctx,     config.are_inputs_on_device, config.are_outputs_on_device, input_block_size, 8, 72, 9, 0,
      config.is_async};
    return Keccak().hash_many(input, (uint64_t*)output, number_of_states, cfg);
  }
} // namespace keccak