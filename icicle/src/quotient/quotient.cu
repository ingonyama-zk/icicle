...
namespace quotient {
  namespace {
    ...
    __device__ void denominator_inverse(
      ColumnSampleBatch<QP, QF>* sample_batches, uint32_t sample_size, const P domain_point, CF* flat_denominators)
    {
    ...
    QF* batch_random_coeffs)
  {
    ...
    if (tid < sample_size) {
      ...
      for (size_t j = 0; j < sample_batches[tid].size; ++j) {
        ...
        complex_conjugate_line_coeffs<QP, QF, F>(
          point, value, alpha, &flattened_line_coeffs[sampled_offset], &flattened_line_coeffs[sampled_offset + 1],
    ...
    QF* result)
  {
    ...
    if (row < domain_size) {
      ...
      denominator_inverse<QP, QF, CF>(samples, sample_size, point, denominator_inverses_local);
  ...
  template <typename QP, typename QF>
  __global__ void set_columns_and_values_pointers(
    ...
    QF* result)
  {
    ...
    if (cfg.are_columns_on_device) {
      ...
      CHK_IF_RETURN(cudaMemcpyAsync(
        d_columns, columns, sizeof(F) * number_of_columns * domain_size, cudaMemcpyHostToDevice, stream));
    }
    ColumnSampleBatch<QP, QF>* d_samples;
    uint32_t** d_columns_ptrs;
    QF** d_values_ptrs;
    ...
    if (cfg.are_sample_points_on_device) {
      ...
      for (int i = 0; i < sample_size; ++i) {
        ...
        if (samples[i].size > 0) {
          ...
          cudaMemcpyAsync(
            h_columns_ptrs[i], samples[i].columns, sizeof(uint32_t) * samples[i].size, cudaMemcpyHostToDevice, stream);

          cudaMallocAsync(&h_values_ptrs[i], sizeof(QF) * samples[i].size, stream);
          cudaMemcpyAsync(
            h_values_ptrs[i], samples[i].values, sizeof(QF) * samples[i].size, cudaMemcpyHostToDevice, stream);
        } else {
      ...
      CHK_IF_RETURN(cudaMemcpyAsync(
        d_columns_ptrs, h_columns_ptrs, sizeof(uint32_t*) * sample_size, cudaMemcpyHostToDevice, stream));
      CHK_IF_RETURN(
        cudaMemcpyAsync(d_values_ptrs, h_values_ptrs, sizeof(QF*) * sample_size, cudaMemcpyHostToDevice, stream));
      CHK_IF_RETURN(
        cudaMemcpyAsync(d_point_ptrs, h_point_ptrs, sizeof(QP*) * sample_size, cudaMemcpyHostToDevice, stream));

      // Copy the struct array from host to device (with placeholder pointers)
      CHK_IF_RETURN(cudaMemcpyAsync(
        d_samples, samples, sizeof(ColumnSampleBatch<QP, QF>) * sample_size, cudaMemcpyHostToDevice, stream));

      // Kernel to set the `columns` and `values` pointers in the device struct array
      set_columns_and_values_pointers<QP, QF><<<(sample_size + 255) / 256, 256, 0, stream>>>(
        d_samples, d_columns_ptrs, d_values_ptrs, d_point_ptrs, sample_size);
    }
    ...
    column_line_and_batch_random_coeffs<QP, QF, F><<<num_blocks, block_dim, 0, stream>>>(
      d_samples, sample_size, random_coefficient, d_flattened_line_coeffs, d_line_coeffs_sizes, d_batch_random_coeffs);
    ...
    accumulate_quotients_kernel<QP, QF, CF, F, P, D><<<num_blocks, block_dim, 0, stream>>>(
      domain, domain_size, d_columns, number_of_columns, random_coefficient, d_samples, sample_size,
      d_flattened_line_coeffs, d_line_coeffs_sizes, d_batch_random_coeffs, d_denominator_inverses, d_result);
    ...
    if (!cfg.are_columns_on_device) { CHK_IF_RETURN(cudaFreeAsync(d_columns, stream)); }
