...
namespace quotient {
    ...
    q_extension_t* result)
  {
    ...
    return accumulate_quotients<secure_point_t, q_extension_t, c_extension_t, scalar_t, point_t, domain_t>(
      domain, columns, number_of_columns, random_coefficient, samples, sample_size, flattened_line_coeffs_size, cfg,
      result);
  }

  extern "C" cudaError_t CONCAT_EXPAND(FIELD, evaluate_constraint_quotients_on_domain)(
