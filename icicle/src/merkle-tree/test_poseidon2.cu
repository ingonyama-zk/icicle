...
int main(int argc, char* argv[])
{
  ...
  poseidon2::Poseidon2<scalar_t> poseidon(
    T, poseidon2::MdsType::DEFAULT_MDS, poseidon2::DiffusionStrategy::DEFAULT_DIFFUSION, ctx);
  ...
  size_t digests_len = merkle_tree::get_digests_len(keep_rows - 1, T);
  ...
  babybear_build_poseidon2_merkle_tree(leaves, digests, tree_height, T, &poseidon, &poseidon, tree_config);
