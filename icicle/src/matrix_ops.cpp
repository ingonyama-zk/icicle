...
namespace icicle {
  ...
  extern "C" eIcicleError CONCAT_EXPAND(ICICLE_FFI_PREFIX, ring_rns_matrix_transpose)(
  ...
  eIcicleError matrix_transpose(
    const scalar_rns_t* mat_in, uint32_t nof_rows, uint32_t nof_cols, const VecOpsConfig& config, scalar_rns_t* mat_out)
  {
    return CONCAT_EXPAND(ICICLE_FFI_PREFIX, ring_rns_matrix_transpose)(mat_in, nof_rows, nof_cols, &config, mat_out);
  }
#endif // RING
} // namespace icicle