...
int main(int argc, char** argv)
{
  ...
  // {
    ...
    // scalars[i] = i>1000? scalars[i-1000] : test_scalar::rand_host();
    // points[i] = i>1000? points[i-1000] : test_projective::to_affine(test_projective::rand_host());
  ...
  msm::msm<test_scalar, test_affine, test_projective>(
    scalars, precomp_factor > 1 ? precomp_points_d : points, chunk_size, config, res_d, &buckets_d);
  ...
  for (int i = 0; i < nof_chunks; i++)
  {
    ...
    msm::msm<test_scalar, test_affine, test_projective>(
    (config.are_scalars_on_device? scalars_d : scalars) + (i%2)*chunk_size, (config.are_points_on_device? points_d : points) + (i%2)*chunk_size, sub_msm_size, config, res_d, &buckets_d);
    ...
    cudaMemcpyAsync(points_d + ((i+1)%2)*chunk_size, points + (i+1)*chunk_size, sizeof(test_affine) * sub_msm_size, cudaMemcpyHostToDevice, transfer_stream);
  ...
  for (int i = 0; i < nof_chunks; i++) {
    ...
    if (sub_msm_size == 0) sub_msm_size = chunk_size;
    msm::msm<test_scalar, test_affine, test_projective>(
    scalars + chunk_size * i, points + chunk_size * i, sub_msm_size, config, ref_d+i, &buckets_d);
  }
