...
public:
  Blake3BackendCPU(uint64_t input_chunk_size)
      : HashBackend("Blake3-CPU", BLAKE3_OUTBYTES, input_chunk_size) {}
  ...
  eIcicleError hash(const std::byte *input, uint64_t size, const HashConfig &config, std::byte *output) const override {
    ...
    for (unsigned batch_idx = 0; batch_idx < config.batch; ++batch_idx) {
      ...
      if (batch_idx == config.batch - 1) {
        batch_size = size - batch_idx * single_input_size;
      }
