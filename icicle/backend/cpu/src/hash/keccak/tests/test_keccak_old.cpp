...
int main(void)
{
  ...
  {
    uint8_t hash[keccak.m_total_output_limbs * sizeof(limb_t)];
    ...
    keccak.run_single_hash((limb_t*)test_string, (limb_t*)hash, config);
    ...
    computed_hash_str[keccak.m_total_output_limbs * sizeof(limb_t) * 2] = '\0'; // Null terminator
