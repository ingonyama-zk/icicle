...
const unsigned Keccak::keccakf_rotc[24] = {
    1, 3, 6, 10, 15, 21, 28, 36, 45, 55, 2, 14, 27, 41, 56, 8, 25, 43, 62,
    18, 39, 61, 20, 44
};

const unsigned Keccak::keccakf_piln[24] = {
...
sha3_return_t
Keccak::sha3_Init(sha3_context *priv, unsigned bitSize) const{
...
void
Keccak::sha3_Init256(sha3_context *priv) const
{
    Keccak::sha3_Init(priv, 256);
}

void
Keccak::sha3_Init384(sha3_context *priv) const
{
    Keccak::sha3_Init(priv, 384);
}

void
Keccak::sha3_Init512(sha3_context *priv) const
{
    Keccak::sha3_Init(priv, 512);
}

enum SHA3_FLAGS
Keccak::sha3_SetFlags(sha3_context *priv, enum SHA3_FLAGS flags) const
{
    sha3_context *ctx = (sha3_context *) priv;
...
void
Keccak::sha3_Update(sha3_context *priv, void const *bufIn, size_t len) const
{
    sha3_context *ctx = (sha3_context *) priv;
 ...
 */
void const *
Keccak::sha3_Finalize(sha3_context *priv)const
{
    sha3_context *ctx = (sha3_context *) priv;
...
sha3_return_t Keccak::sha3_HashBuffer( unsigned bitSize, enum SHA3_FLAGS flags, const void *in, unsigned inBytes, void *out, unsigned outBytes ) const{
    sha3_return_t err;
    sha3_context c;
    ...
    const limb_t* secondery_input_limbs) const
  {
    ...
    int result = sha3_HashBuffer( this->bitSize, this->sha_flag, input_bytes,this->m_total_input_limbs*sizeof(limb_t), output_bytes, this->m_total_output_limbs*sizeof(limb_t));
