...
namespace icicle {
  ...
  template <typename E>
  eIcicleError
  lookups_constraint_op(const Device& device, const LookupData<E>& lookup_data, const LookupConfig& config, E* result)
  {
    for (uint32_t idx = 0; idx < lookup_data.num_elements; idx++) {
      ...
      int r_prev = ((idx - lookup_data.rot_scale) % lookup_data.i_size + lookup_data.i_size) % lookup_data.i_size;
    ...
    E* result)
  {
    ...
    auto get_value = [&](int source_type, size_t column_index, size_t rotation_index) -> E {
      ...
      switch (source_type) {
      case 0:
        return calc_data.constants[column_index];
      case 1:
        return intermediates[column_index];
      case 2:
        return gate_data.fixed[column_index * gate_data.num_fixed_rows + row];
      case 3:
        return gate_data.advice[column_index * gate_data.num_advice_rows + row];
      case 4:
        return gate_data.instance[column_index * gate_data.num_instance_rows + row];
      case 5:
        return gate_data.challenges[column_index];
      case 6:
        return gate_data.beta[0];
      case 7:
        return gate_data.gamma[0];
      case 8:
        return gate_data.theta[0];
      case 9:
        return gate_data.y[0];
      case 10:
        return calc_data.previous_value[column_index];
      default:
        return E::zero();
      }
    };
    ...
    auto get_result = [&](int calc_type, size_t calc_idx) -> E {
      E first = get_value(
        calc_data.value_types[calc_idx * 2], calc_data.value_indices[calc_idx * 4],
        calc_data.value_indices[calc_idx * 4 + 1]);
      E second = get_value(
        calc_data.value_types[calc_idx * 2 + 1], calc_data.value_indices[calc_idx * 4 + 2],
      ...
      case 3:
        return E::sqr(first); // Square
      case 4:
        return first + first; // Double
      case 5:
        return E::neg(first); // Negate
      case 6: {               // Horner
        ...
        for (uint32_t i = offset; i < offset + size; i++) {
          E part_value = get_value(
            horner_data.value_types[i], horner_data.value_indices[i * 2], horner_data.value_indices[i * 2 + 1]);
    ...
    for (uint32_t idx = 0; idx < calc_data.num_elements; idx++) {
      ...
      result[idx] =
        intermediates[idx * calc_data.num_intermediates + calc_data.targets[calc_data.num_calculations - 1]];
    }
