...
int get_nof_workers()
{
  ...
  // Note: no need to account for the main thread in gate-ops since it's doing little work
...
eIcicleError lookups_constraint_op(
    ...
    E* result)
{
    // Use Taskflow for parallelization
    tf::Taskflow taskflow;
    tf::Executor executor;
    
    const int nof_workers = get_nof_workers();
    const uint64_t worker_task_size = (lookup_data.num_elements + nof_workers - 1) / nof_workers; // round up

    for (uint64_t start_idx = 0; start_idx < lookup_data.num_elements; start_idx += worker_task_size) {
        taskflow.emplace([=]() {
            const uint64_t end_idx = std::min(start_idx + worker_task_size, static_cast<uint64_t>(lookup_data.num_elements));
    ...
    E* result)
{
    ...
    const int nof_workers = get_nof_workers();
    const uint64_t worker_task_size = (calc_data.num_elements + nof_workers - 1) / nof_workers; // round up

    for (uint64_t start_idx = 0; start_idx < calc_data.num_elements; start_idx += worker_task_size) {
        taskflow.emplace([=]() {
            const uint64_t end_idx = std::min(start_idx + worker_task_size, static_cast<uint64_t>(calc_data.num_elements));
            
            // Allocate memory for intermediates for this worker
            E* intermediates = new E[calc_data.num_intermediates];
            uint32_t* rotation_indices = new uint32_t[calc_data.num_rotations];

            for (uint32_t idx = start_idx; idx < end_idx; idx++) {
                // Precompute rotation indices for this element
                for (uint32_t rot_idx = 0; rot_idx < calc_data.num_rotations; rot_idx++) {
                    int rot = calc_data.rotations[rot_idx];
                    rotation_indices[rot_idx] = 
                ...
                auto get_value = [&](int source_type, size_t column_index, size_t rotation_index) -> E {
                    size_t row = rotation_indices[rotation_index];
                    switch (source_type) {
                        ...
                        case 10: return calc_data.is_prev_zero ? E::zero() : calc_data.previous_value[idx];
                ...
                for (uint32_t calc_idx = 0; calc_idx < calc_data.num_calculations; calc_idx++) {
                    ...
                    intermediates[target_idx] = res;
                }

                result[idx] = intermediates[calc_data.targets[calc_data.num_calculations - 1]];
            }
