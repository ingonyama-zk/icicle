...
class Msm {
  public:
  // Constructor
  Msm(const int msm_size, const MSMConfig& config) :
    m_msm_size(msm_size),
    m_config(config) {
  ...
  private: 
    ...
    uint32_t           m_c;
    uint32_t           m_nof_workers;
    uint32_t           m_nof_buckets_module;
    uint32_t           m_bm_size;
    uint64_t           m_nof_total_buckets;
    uint32_t           m_precompute_factor;
    ...
    void init_workers_buckets_busy() {
    ...
    void worker_run_phase1 (const int worker_idx, const scalar_t* scalars, const A* bases, const unsigned int msm_size) {
      ...
      for (int i = 0; i < msm_size; i++) {
        ...
        scalar_t scalar = m_config.are_scalars_montgomery_form ? scalar_t::from_montgomery(scalars[i]) : scalars[i]; // TBD: avoid copy
        ...
        for (int j = 0; j < m_precompute_factor; j++) {
          ...
          A base =
            m_config.are_points_montgomery_form ? A::from_montgomery(bases[m_precompute_factor * i + j]) : bases[m_precompute_factor * i + j]; // TDB: avoid copy
          ...
          for (int bm_i = 0; bm_i < m_nof_buckets_module; bm_i++) {
            ...
            if ((curr_coeff & coeff_bit_mask_with_sign_bit) != 0) {
              ...
              if (buckets_busy[bkt_idx]) {
                buckets[bkt_idx].point = buckets[bkt_idx].point + ((negate_p_and_s ^ (carry > 0)) ? base_neg : base); // TBD: inplace
              } else {
                buckets_busy[bkt_idx] = true;
                buckets[bkt_idx].point = P::from_affine(((negate_p_and_s ^ (carry > 0)) ? base_neg : base));  // TBD: inplace
              }
            } else {
  ...
  //      #pragma omp parallel for
      ...
      for(int bucket_i = bucket_start; bucket_i < bucket_start + nof_buckets; bucket_i++) {
        for(int worker_i = 1; worker_i < m_nof_workers; worker_i++) {
          if (m_workers_buckets_busy[worker_i][bucket_i]) {
            m_workers_buckets[0][bucket_i].point = m_workers_buckets_busy[0][bucket_i] ? m_workers_buckets[0][bucket_i].point + m_workers_buckets[worker_i][bucket_i].point : // TBD inplace
                                                                    m_workers_buckets[worker_i][bucket_i].point; // copy the point to worker[0]
    ...
    void worker_collapse_segment(Segment& segment, const int64_t bucket_start, const uint32_t segment_size) {
      ...
      std::vector<bool>& buckets_busy = m_workers_buckets_busy[0];
      const int64_t last_bucket_i = bucket_start + segment_size;
      const int64_t init_bucket_i = (last_bucket_i % m_bm_size) ? last_bucket_i :       // bucket 0 at the BM contains the last element
                                    last_bucket_i - m_bm_size;
      segment.triangle_sum = init_bucket_i < buckets.size() && buckets_busy[init_bucket_i] ? buckets[init_bucket_i].point : P::zero();
    ...
    void phase3_final_accumulator(P* result) {
      ...
      for (int i = 0; i < m_nof_buckets_module; i++) {
        ...
        // std::cout << "i=" << i << ", m_segments[cur_bm_nof_segments * i].line_sum = " << m_segments[cur_bm_nof_segments * i].line_sum.to_affine() << std::endl;
        ...
        // std::cout << "i=" << i << ", m_segments[cur_bm_nof_segments * i].triangle_sum = " << m_segments[cur_bm_nof_segments * i].triangle_sum.to_affine() << std::endl;
        // After which add the lines and triangle sums to one sum of the entire BM
        if (cur_bm_nof_segments > 1) {
        // std::cout << "i=" << i << ", adding = " << m_segments[cur_bm_nof_segments * i].line_sum.to_affine() << std::endl;
          m_segments[cur_bm_nof_segments * i].triangle_sum =
...
{
  int c = 13; // config.c;  // TBD fix that
