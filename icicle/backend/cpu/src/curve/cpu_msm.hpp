...
private:
      ...
      for (int worker_i = 1; worker_i < m_nof_workers; worker_i++) {
        if (m_workers_buckets_busy[worker_i][bucket_i]) {
          m_workers_buckets[0][bucket_i].point =
            m_workers_buckets_busy[0][bucket_i]
              ? m_workers_buckets[0][bucket_i].point + m_workers_buckets[worker_i][bucket_i].point
              :                                            // TBD inplace
  ...
  void worker_collapse_segment(Segment& segment, const int64_t bucket_start, const uint32_t segment_size)
  {
    ...
    segment.triangle_sum =
      init_bucket_i < buckets.size() && buckets_busy[init_bucket_i] ? buckets[init_bucket_i].point : P::zero();
