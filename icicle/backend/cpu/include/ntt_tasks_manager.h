...
namespace ntt_cpu {
  ...
  {
  public:
    NttTasksManagerRef(const NttSubLogn& ntt_sub_logn_ref, uint32_t logn);

    // Add a new task to the ntt_task_manager
    eIcicleError push_task(const NttTaskCoordinates& ntt_task_coordinates);

    // Set a task as completed and update dependencies
    bool handle_completed(NttTaskRef<S, E>* completed_task, uint32_t nof_subntts_l1);
    NttTaskCoordinates* get_slot_for_next_task_coordinates();
   ...
   */
  template <typename S, typename E>
  NttTasksManagerRef<S, E>::NttTasksManagerRef(const NttSubLogn& ntt_sub_logn_ref, uint32_t logn)
      : ntt_sub_logn(ntt_sub_logn_ref),
        counters(logn > HIERARCHY_1 ? 2 : 1, TasksDependenciesCounters(ntt_sub_logn_ref, 0)),
        task_buffer(1 << (logn)), // Pre-allocate buffer
        head(0), tail(0), nof_pending_tasks(0)
   ...
   */
  template <typename S, typename E>
  bool NttTasksManagerRef<S, E>::is_full() const
  {
    return (tail + 1) % (1 << (ntt_sub_logn.logn)) == head;
  }
   ...
   */
  template <typename S, typename E>
  bool NttTasksManagerRef<S, E>::is_empty() const
  {
    return head == tail && nof_pending_tasks == 0;
  }
   ...
   */
  template <typename S, typename E>
  void NttTasksManagerRef<S, E>::increment(size_t& index)
  {
    index = (index + 1) % (1 << (ntt_sub_logn.logn));
  }
   ...
   */
  template <typename S, typename E>
  void NttTasksManagerRef<S, E>::decrement(size_t& index)
  {
    index = (index - 1) % (1 << (ntt_sub_logn.logn));
  }
   ...
   */
  template <typename S, typename E>
  eIcicleError NttTasksManagerRef<S, E>::push_task(const NttTaskCoordinates& ntt_task_coordinates)
  {
    if (is_full()) { return eIcicleError::OUT_OF_MEMORY; }
    task_buffer[tail] = ntt_task_coordinates;
    increment(tail);
   ...
   */
  template <typename S, typename E>
  NttTaskCoordinates* NttTasksManagerRef<S, E>::get_slot_for_next_task_coordinates()
  {
    if (is_full()) { return nullptr; }
    NttTaskCoordinates* task = &task_buffer[tail];
    increment(tail);
   ...
   */
  template <typename S, typename E>
  NttTaskCoordinates* NttTasksManagerRef<S, E>::get_available_task()
  {
    if (head == tail) {
      // No available tasks
      return nullptr;
   ...
   */
  template <typename S, typename E>
  bool NttTasksManagerRef<S, E>::tasks_to_do() const
  {
    return head != tail || nof_pending_tasks != 0;
  }
   ...
   */
  template <typename S, typename E>
  bool NttTasksManagerRef<S, E>::available_tasks() const
  {
    return head != tail;
  }
   ...
   */
  template <typename S, typename E>
  bool NttTasksManagerRef<S, E>::handle_completed(NttTaskRef<S, E>* completed_task, uint32_t nof_subntts_l1)
  {
    bool task_dispatched = false;
    NttTaskCoordinates task_c = *completed_task->get_coordinates();
    uint32_t nof_hierarchy_0_layers = counters[task_c.hierarchy_1_layer_idx].get_nof_hierarchy_0_layers();
