...
namespace ntt_cpu {
  ...
  eIcicleError NttTask<S, E>::reorder_and_refactor_if_needed()
  {
    ...
    for (uint32_t row_batch = 0; row_batch < rows_batch_reps;
      ...
      uint64_t tw_idx = 0;
      const S* twiddles = CpuNttDomain<S>::s_ntt_domain.get_twiddles_mont();
  ...
  {
    // ICICLE_LOG_DEBUG << "NttTask<S, E>::ntt8win() // MONT";
    ...
    const S* twiddles = ntt_data->direction == NTTDir::kForward
                          ? CpuNttDomain<S>::s_ntt_domain.get_winograd8_twiddles_mont()
                          : CpuNttDomain<S>::s_ntt_domain.get_winograd8_twiddles_inv_mont();
    ...
    for (uint32_t batch = 0; batch < ntt_data->config.batch_size; ++batch) {
      ...
      if (
        ...
        apply_coset_multiplication(current_elements, index_in_mem, CpuNttDomain<S>::s_ntt_domain.get_twiddles_mont());
      }
  ...
  {
    // ICICLE_LOG_DEBUG << "NttTask<S, E>::ntt16win() , mont";
    uint32_t offset = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    E* subntt_elements =
      ntt_data->elements +
      offset * (ntt_task_coordinates->hierarchy_1_subntt_idx
                << ntt_data->ntt_sub_logn
                     .hierarchy_1_layers_sub_logn[ntt_task_coordinates->hierarchy_1_layer_idx]); // input + subntt_idx *
                                                                                                 // subntt_size
    const S* twiddles = ntt_data->direction == NTTDir::kForward
                          ? CpuNttDomain<S>::s_ntt_domain.get_winograd16_twiddles_mont()
                          : CpuNttDomain<S>::s_ntt_domain.get_winograd16_twiddles_inv_mont();
      ...
      offset * (ntt_task_coordinates->hierarchy_1_subntt_idx
                << ntt_data->ntt_sub_logn
                     .hierarchy_1_layers_sub_logn[ntt_task_coordinates->hierarchy_1_layer_idx]); // input + subntt_idx *
                                                                                                 // subntt_size
    const S* twiddles = ntt_data->direction == NTTDir::kForward
                          ? CpuNttDomain<S>::s_ntt_domain.get_winograd32_twiddles_mont()
                          : CpuNttDomain<S>::s_ntt_domain.get_winograd32_twiddles_inv_mont();
  ...
  {
    // ICICLE_LOG_DEBUG << "NttTask<S, E>::hierarchy_0_dit_ntt() // MONT";
    uint32_t offset = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    E* subntt_elements =
      ntt_data->elements +
      offset * (ntt_task_coordinates->hierarchy_1_subntt_idx
   ...
   */
  template <typename S, typename E>
  eIcicleError NttTaskRef<S, E>::reorder_and_refactor_if_needed()
  {
    uint32_t columns_batch_reps = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    uint32_t rows_batch_reps = ntt_data->config.columns_batch ? 1 : ntt_data->config.batch_size;
    for (uint32_t row_batch = 0; row_batch < rows_batch_reps;
          ...
          if (refactor_pre_hierarchy_1_next_layer) {
            tw_idx = (ntt_data->direction == NTTDir::kForward)
                       ...
                       : CpuNttDomain<S>::s_ntt_domain.get_max_size() -
                           ((CpuNttDomain<S>::s_ntt_domain.get_max_size() >> ntt_data->ntt_sub_logn.logn) *
                            ntt_task_coordinates->hierarchy_1_subntt_idx * new_idx);
            current_temp_output[stride * new_idx] = original_multiplier(current_elements[stride * i], twiddles[tw_idx]);
          } else {
            current_temp_output[stride * new_idx] = current_elements[stride * i];
          }
        }
   ...
   */
  template <typename S, typename E>
  void NttTaskRef<S, E>::apply_coset_multiplication(
    E* current_elements, const std::vector<uint32_t>& index_in_mem, const S* twiddles)
  {
    uint32_t current_subntt_size =
      1 << ntt_data->ntt_sub_logn.hierarchy_0_layers_sub_logn[ntt_task_coordinates->hierarchy_1_layer_idx]
    ...
    for (uint32_t i = 0; i < current_subntt_size; i++) {
      ...
      if (ntt_data->arbitrary_coset) {
        current_elements[index_in_mem[new_idx]] =
          original_multiplier(current_elements[index_in_mem[new_idx]], ntt_data->arbitrary_coset[idx]);
      } else {
        uint32_t twiddle_idx = ntt_data->coset_stride * idx;
        twiddle_idx = ntt_data->direction == NTTDir::kForward
                        ? twiddle_idx
                        : CpuNttDomain<S>::s_ntt_domain.get_max_size() - twiddle_idx;
        current_elements[index_in_mem[new_idx]] = original_multiplier(current_elements[index_in_mem[new_idx]], twiddles[twiddle_idx]);
      }
    }
  }
   ...
   */
  template <typename S, typename E>
  eIcicleError NttTaskRef<S, E>::hierarchy_0_cpu_ntt()
  {
    const uint64_t subntt_size =
      (1 << ntt_data->ntt_sub_logn.hierarchy_0_layers_sub_logn[ntt_task_coordinates->hierarchy_1_layer_idx]
                                                              [ntt_task_coordinates->hierarchy_0_layer_idx]);
   ...
   */
  template <typename S, typename E>
  void NttTaskRef<S, E>::ntt8win() // N --> N
  {
    // ICICLE_LOG_DEBUG << "NttTaskRef<S, E>::ntt8win() // BARET" ;
    uint32_t offset = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    E* subntt_elements =
      ntt_data->elements +
      offset * (ntt_task_coordinates->hierarchy_1_subntt_idx
    ...
    for (uint32_t batch = 0; batch < ntt_data->config.batch_size; ++batch) {
      ...
      current_elements[index_in_mem[2]] = original_multiplier(current_elements[index_in_mem[2]], twiddles[0]);
      ...
      current_elements[index_in_mem[1]] = original_multiplier(current_elements[index_in_mem[1]], twiddles[1]);
      current_elements[index_in_mem[5]] = original_multiplier(current_elements[index_in_mem[5]], twiddles[0]);
      current_elements[index_in_mem[3]] = original_multiplier(current_elements[index_in_mem[3]], twiddles[2]);
      ...
      if (last_layer && ntt_data->direction == NTTDir::kInverse) {
        S inv_size = S::inv_log_size(ntt_data->ntt_sub_logn.logn);
        for (uint64_t i = 0; i < 8; ++i) {
          current_elements[index_in_mem[i]] = original_multiplier(current_elements[index_in_mem[i]], inv_size);
        }
      }
    }
  }
   ...
   */
  template <typename S, typename E>
  void NttTaskRef<S, E>::ntt16win() // N --> N
  {
    // ICICLE_LOG_DEBUG << "NttTaskRef<S, E>::ntt16win() // BARET" ;
    uint32_t offset = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    E* subntt_elements =
      ntt_data->elements +
      offset * (ntt_task_coordinates->hierarchy_1_subntt_idx
    ...
    for (uint32_t batch = 0; batch < ntt_data->config.batch_size; ++batch) {
      ...
      current_elements[index_in_mem[11]] = current_elements[index_in_mem[7]] + current_elements[index_in_mem[15]];
      current_elements[index_in_mem[7]] = current_elements[index_in_mem[7]] - current_elements[index_in_mem[15]];
      current_elements[index_in_mem[4]] = original_multiplier(twiddles[3], current_elements[index_in_mem[4]]);
      ...
      current_elements[index_in_mem[12]] = original_multiplier(twiddles[5] , current_elements[index_in_mem[12]]);
      current_elements[index_in_mem[10]] = original_multiplier(twiddles[6] , current_elements[index_in_mem[10]]);
      current_elements[index_in_mem[2]] = original_multiplier(twiddles[7] , current_elements[index_in_mem[2]]);
      ...
      current_elements[index_in_mem[13]] = current_elements[index_in_mem[11]] + current_elements[index_in_mem[7]];
      current_elements[index_in_mem[13]] = original_multiplier(twiddles[14] , current_elements[index_in_mem[13]]);
      current_elements[index_in_mem[11]] =
        original_multiplier(twiddles[12] , current_elements[index_in_mem[11]]) + current_elements[index_in_mem[13]];
      current_elements[index_in_mem[7]] =
        original_multiplier(twiddles[13] , current_elements[index_in_mem[7]]) + current_elements[index_in_mem[13]];

      current_elements[index_in_mem[13]] = current_elements[index_in_mem[1]] + current_elements[index_in_mem[3]];
      current_elements[index_in_mem[13]] = original_multiplier(twiddles[17] , current_elements[index_in_mem[13]]);
      current_elements[index_in_mem[1]] =
        original_multiplier(twiddles[15] , current_elements[index_in_mem[1]]) + current_elements[index_in_mem[13]];
      current_elements[index_in_mem[3]] =
        original_multiplier(twiddles[16] , current_elements[index_in_mem[3]]) + current_elements[index_in_mem[13]];
      ...
      current_elements[index_in_mem[6]] = original_multiplier(twiddles[9] , current_elements[index_in_mem[6]]);
      current_elements[index_in_mem[9]] = original_multiplier(twiddles[10] , current_elements[index_in_mem[9]]);
      current_elements[index_in_mem[14]] = original_multiplier(twiddles[11] , current_elements[index_in_mem[14]]);
   ...
   */
  template <typename S, typename E>
  void NttTaskRef<S, E>::ntt32win() // N --> N
  {
    // ICICLE_LOG_DEBUG << "NttTaskRef<S, E>::ntt8win() // BARET" ;
    uint32_t offset = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    E* subntt_elements =
    ...
    for (uint32_t batch = 0; batch < ntt_data->config.batch_size; ++batch) {
      ...
      temp_0[3] = original_multiplier(temp_1[3], twiddles[3]);
      temp_0[4] = temp_1[4];
      for (uint32_t i = 5; i < 8; i++) {
        temp_0[i] = original_multiplier(temp_1[i], twiddles[i]);
      }
      temp_0[8] = temp_1[8];
      for (uint32_t i = 9; i < 18; i++) {
        temp_0[i] = original_multiplier(temp_1[i], twiddles[i]);
   ...
   */
  template <typename S, typename E>
  void NttTaskRef<S, E>::hierarchy_0_dit_ntt() // R --> N
  {
    // ICICLE_LOG_DEBUG << "NttTaskRef<S, E>::hierarchy_0_dit_ntt() // BARET";
    uint32_t offset = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    E* subntt_elements =
      ntt_data->elements +
      offset * (ntt_task_coordinates->hierarchy_1_subntt_idx
          ...
          for (uint32_t j = 0; j < half_len; ++j) {
            ...
            if (j == 0) {
              ...
              uint32_t tw_idx = (ntt_data->direction == NTTDir::kForward)
                                  ? j * step
                                  : CpuNttDomain<S>::s_ntt_domain.get_max_size() - j * step;
              v = original_multiplier(current_elements[v_mem_idx], twiddles[tw_idx]);
            }
            current_elements[u_mem_idx] = u + v;
            current_elements[v_mem_idx] = u - v;
          }
   ...
   */
  template <typename S, typename E>
  void NttTaskRef<S, E>::hierarchy_0_dif_ntt() // N --> R
  {
    uint32_t offset = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    E* subntt_elements =
      ntt_data->elements +
          ...
          for (uint32_t j = 0; j < half_len; ++j) {
            ...
            if (j == 0) {
              ...
              uint32_t tw_idx = (ntt_data->direction == NTTDir::kForward)
                                  ? j * step
                                  : CpuNttDomain<S>::s_ntt_domain.get_max_size() - j * step;
              current_elements[v_mem_idx] = original_multiplier((u - v), twiddles[tw_idx]);
            }
          }
        }
      }
   ...
   */
  template <typename S, typename E>
  void NttTaskRef<S, E>::reorder_by_bit_reverse()
  {
    uint32_t offset = ntt_data->config.columns_batch ? ntt_data->config.batch_size : 1;
    E* subntt_elements =
      ntt_data->elements +
   ...
   */
  template <typename S, typename E>
  void NttTaskRef<S, E>::refactor_output_hierarchy_0()
  {
    uint32_t hierarchy_0_subntt_size =
      1 << ntt_data->ntt_sub_logn.hierarchy_0_layers_sub_logn[ntt_task_coordinates->hierarchy_1_layer_idx]
                                                             [ntt_task_coordinates->hierarchy_0_layer_idx];
    ...
    for (uint32_t batch = 0; batch < ntt_data->config.batch_size; ++batch) {
      ...
      for (uint32_t elem = 0; elem < hierarchy_0_subntt_size; elem++) {
        ...
        uint64_t tw_idx = (ntt_data->direction == NTTDir::kForward)
                            ...
                            : CpuNttDomain<S>::s_ntt_domain.get_max_size() -
                                ((CpuNttDomain<S>::s_ntt_domain.get_max_size() / ntt_size) * j * i);
        elements_of_current_batch[elem_mem_idx] = original_multiplier(elements_of_current_batch[elem_mem_idx], twiddles[tw_idx]);
      }
    }
  }
  ...
  template <typename S, typename E>
  uint64_t NttTaskRef<S, E>::idx_in_mem(NttTaskCoordinates* ntt_task_coordinates, uint32_t element_idx)
  {
    uint32_t s0 = ntt_data->ntt_sub_logn.hierarchy_0_layers_sub_logn[ntt_task_coordinates->hierarchy_1_layer_idx][0];
    uint32_t s1 = ntt_data->ntt_sub_logn.hierarchy_0_layers_sub_logn[ntt_task_coordinates->hierarchy_1_layer_idx][1];
    uint32_t s2 = ntt_data->ntt_sub_logn.hierarchy_0_layers_sub_logn[ntt_task_coordinates->hierarchy_1_layer_idx][2];
