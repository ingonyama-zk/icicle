...
namespace ntt_cpu {
   ...
   *
   * `layers_sub_logn_ref` specifies the log sizes for up to three layers (hierarchy1 or hierarchy0) in the NTT
   * computation.
   ...
   *
   * Example: `layers_sub_logn_ref[14] = {14, 13, 0}` means for `logn = 14`, the sub-NTT log sizes are 14 for the first
   ...
   *
   * @method NttSubLognRef(int logn) Initializes the struct based on the given `logn`.
   */
    ...
    NttSubLognRef(int logn) : logn(logn)
    {
      ...
      if (logn > HIERARCHY_1_REF) {
        ...
        hierarchy_0_layers_sub_logn = {
          std::vector<int>(
            std::begin(layers_sub_logn_ref[hierarchy_1_layers_sub_logn[0]]),
            std::end(layers_sub_logn_ref[hierarchy_1_layers_sub_logn[0]])),
          std::vector<int>(
            std::begin(layers_sub_logn_ref[hierarchy_1_layers_sub_logn[1]]),
            std::end(layers_sub_logn_ref[hierarchy_1_layers_sub_logn[1]]))};
      } else {
  ...
  public:
    ...
    eIcicleError hierarchy1_push_tasks(
    ...
    eIcicleError handle_pushed_tasks(
      TasksManager<NttTaskRef<S, E>>* tasks_manager,
      NttTasksManagerRef<S, E>& ntt_tasks_manager,
   ...
   *
   * @method TasksDependenciesCountersRef(NttSubLognRef ntt_sub_logn, int hierarchy_1_layer_idx) Constructor that
   * initializes the counters based on NTT structure.
   * @method bool decrement_counter(NttTaskCoordinatesRef ntt_task_coordinates) Decrements the counter for a given task
   * and returns true if the task is ready to execute.
   ...
   *
   * The NttTasksManagerRef is responsible for adding tasks, updating task dependencies,
   ...
   */
  TasksDependenciesCountersRef::TasksDependenciesCountersRef(NttSubLognRef ntt_sub_logn, int hierarchy_1_layer_idx)
  ...
  template <typename S, typename E>
  eIcicleError
  NttTasksManagerRef<S, E>::push_task(NttCpuRef<S, E>* ntt_cpu, E* input, NttTaskCoordinatesRef task_c, bool reorder)
  {
   ...
   */
  template <typename S, typename E>
  uint64_t NttCpuRef<S, E>::bit_reverse(uint64_t n, int logn)
  {
    int rev = 0;
  ...
  template <typename S, typename E>
  void
  NttCpuRef<S, E>::reorder_by_bit_reverse(NttTaskCoordinatesRef ntt_task_coordinates, E* elements, bool is_top_hirarchy)
  {
    ...
    int subntt_log_size =
      is_top_hirarchy ? (this->ntt_sub_logn.logn)
  ...
  template <typename S, typename E>
  eIcicleError NttCpuRef<S, E>::copy_and_reorder_if_needed(const E* input, E* output)
  {
   ...
   */
  template <typename S, typename E>
  void NttCpuRef<S, E>::coset_mul(E* elements, int coset_stride, const std::unique_ptr<S[]>& arbitrary_coset)
  {
  ...
  template <typename S, typename E>
  int NttCpuRef<S, E>::find_or_generate_coset(std::unique_ptr<S[]>& arbitrary_coset)
  {
    int coset_stride = 0;
   ...
   */
  template <typename S, typename E>
  void NttCpuRef<S, E>::hierarchy_1_reorder(E* elements)
  {
   ...
   */
  template <typename S, typename E>
  eIcicleError NttCpuRef<S, E>::reorder_and_refactor_if_needed(
    E* elements, NttTaskCoordinatesRef ntt_task_coordinates, bool is_top_hirarchy)
  {
  ...
  void NttCpuRef<S, E>::refactor_output_hierarchy_0(E* elements, NttTaskCoordinatesRef ntt_task_coordinates)
  {
    ...
    int i, j, i_0;
    int ntt_size = ntt_task_coordinates.hierarchy_0_layer_idx == 0
                     ? 1
