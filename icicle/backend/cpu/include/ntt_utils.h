  ...
  struct NttTaskCoordinates {
    ...
    bool operator==(const NttTaskCoordinates& other) const
    {
      return hierarchy_1_layer_idx == other.hierarchy_1_layer_idx &&
             ...
             hierarchy_0_subntt_idx == other.hierarchy_0_subntt_idx && reorder == other.reorder;
    }
    ...
    NttTaskCoordinates(
      uint32_t h1_layer_idx,
      uint32_t h1_subntt_idx,
      uint32_t h0_layer_idx,
      uint32_t h0_block_idx,
      uint32_t h0_subntt_idx,
      bool reorder_flag = false)
        : hierarchy_1_layer_idx(h1_layer_idx), hierarchy_1_subntt_idx(h1_subntt_idx),
          hierarchy_0_layer_idx(h0_layer_idx), hierarchy_0_block_idx(h0_block_idx),
          hierarchy_0_subntt_idx(h0_subntt_idx), reorder(reorder_flag)
    {
    }
  };
