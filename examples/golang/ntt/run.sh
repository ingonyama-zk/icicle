...
if [ "$DEVICE_TYPE" != "CPU" ] && [ ! -d "${ICICLE_BACKEND_INSTALL_DIR}" ] && [ -d "${ICICLE_BACKEND_SOURCE_DIR}" ]; then
  ...
  cmake -DCMAKE_BUILD_TYPE=Release -DCURVE=bls12_377 -DECNTT=OFF "-D${DEVICE_TYPE}_BACKEND"=local -S "${ICICLE_DIR}" -B "${BUILD_DIR}"
  cmake --build "${BUILD_DIR}" -j
  ...
  cmake --build "${BUILD_DIR}" -j
  rm "${BUILD_DIR}/CMakeCache.txt"
  cmake -DCMAKE_BUILD_TYPE=Release -DCURVE=bls12_377 -S "${ICICLE_DIR}" -B "${BUILD_DIR}"
  cmake --build "${BUILD_DIR}" -j
fi
