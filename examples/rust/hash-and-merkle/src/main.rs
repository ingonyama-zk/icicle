...
fn keccak_hash_example() {
    ...
    let input_field_elements = BabybearField::generate_random(128); // Generate random field elements
    let mut output = vec![0u8; 32]; // Output buffer for the hash
    ...
    let input_field_elements_batch = BabybearField::generate_random(total_input_elements);
