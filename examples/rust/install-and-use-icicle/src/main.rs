...
use icicle_runtime::{self, eIcicleError, Device};
...
fn main() {
    ...
    let mut input_gpu = DeviceVec::<ScalarField>::device_malloc(ntt_size).unwrap();
    let mut output_gpu = DeviceVec::<ScalarField>::device_malloc(ntt_size).unwrap();
    input_gpu
        .copy_from_host(HostSlice::from_slice(&input_cpu))
        .unwrap();
    ntt(&input_gpu[..], ntt::NTTDir::kForward, &ntt_config, &mut output_gpu[..]).unwrap();
    output_gpu
